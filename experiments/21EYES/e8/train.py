...
def _loss_for_batch(model: torch.nn.Module, batch, device: torch.device, amp_enabled: bool, config: Dict[str, Any]):
    ...
    with _autocast(device, amp_enabled):
        ...
        if variant != "mirrored_four_avenue":
            lambda_align = 0.0
...
def run_training(config_path: Path, args: argparse.Namespace) -> Path:
    ...
    output_root = Path(args.output_root).resolve() if args.output_root is not None else E8_ROOT / "results"
    ...
    if output_root == E8_ROOT / "results":
        write_report(E8_ROOT)
    print(f"Wrote {run_dir / 'final_metrics.json'}")
    return run_dir


def main() -> None:
    parser = argparse.ArgumentParser(description="Train a 21EYES Experiment 8 scout variant")
    ...
    if not config_path.is_absolute():
        config_path = E8_ROOT / config_path

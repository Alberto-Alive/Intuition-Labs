...
from eval import append_jsonl, evaluate_avenue_controls, evaluate_model, save_json, write_report
...
def _resolve_batch_size(value: Any, *, device: torch.device, train: bool, has_avenues: bool) -> int:
    if value is None or str(value).lower() == "auto":
        if device.type == "cuda":
            if has_avenues:
                return 48 if train else 32
            return 64 if train else 32
        return 6 if has_avenues else 8
    return int(value)
...
def _loss_for_batch(model: torch.nn.Module, batch, device: torch.device, amp_enabled: bool, config: Dict[str, Any]):
    ...
    with _autocast(device, amp_enabled):
        ...
        if variant != "e8_best_reference":
            lambda_align = 0.0
        if variant in {"baseline_transformer", "param_matched_baseline"}:
            lambda_div = 0.0
        loss = task_loss + lambda_align * align_loss + lambda_div * div_loss
    return loss, out, {
...
def run_training(config_path: Path, args: argparse.Namespace) -> Path:
    ...
    has_avenues = variant not in {"baseline_transformer", "param_matched_baseline"}
    batch_size = _resolve_batch_size(data_cfg.get("batch_size", "auto"), device=device, train=True, has_avenues=has_avenues)
    eval_batch_size = _resolve_batch_size(data_cfg.get("eval_batch_size", "auto"), device=device, train=False, has_avenues=has_avenues)
    ...
    output_root = Path(args.output_root).resolve() if args.output_root is not None else E9_ROOT / "results"
    ...
    avenue_controls = evaluate_avenue_controls(model, config, device, seed_offset=700_000)
    peak_memory_mb = None
    if device.type == "cuda":
    ...
    if output_root == E9_ROOT / "results":
        write_report(E9_ROOT)
    print(f"Wrote {run_dir / 'final_metrics.json'}")
    return run_dir


def main() -> None:
    parser = argparse.ArgumentParser(description="Train a 21EYES Experiment 9 scout variant")
    ...
    if not config_path.is_absolute():
        config_path = E9_ROOT / config_path

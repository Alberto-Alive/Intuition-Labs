...
E9_ROOT = Path(__file__).resolve().parent
AVENUE_CONTROLS = (
    "avenue_zero",
    "avenue_shuffle",
    "avenue_random",
...
def _extract_layer_metrics(layer_accum: List[Dict[str, float]], denom: int) -> Dict[str, List[Optional[float]]]:
    names = [
        ...
        "avenue_usage_2",
        "avenue_usage_3",
        "junction_attention_entropy",
        "avenue_to_junction_attention_entropy",
        "junction_to_avenue_attention_entropy",
        "junction_effective_source_tokens",
        "junction_token_utilization",
        "junction_norm",
        "junction_diversity",
        "junction_contribution_norm",
        "junction_read_gate",
        "junction_read_scale",
        "final_junction_attention_entropy",
        "final_avenue_to_junction_attention_entropy",
        "final_junction_effective_source_tokens",
        "final_junction_token_utilization",
        "final_junction_norm",
        "final_junction_diversity",
    ]
...
def _update_junction_evidence_diagnostics(
    rank_accum: List[Dict[str, float]],
    junction_attn: Optional[Sequence[Optional[Dict[str, torch.Tensor]]]],
    batch: SyntheticBatch,
) -> None:
    if not junction_attn:
        return
    rel = batch.relevant_positions
    dist = batch.distractor_positions
    queries = batch.query_positions
    for layer_idx, diag in enumerate(junction_attn[: len(rank_accum)]):
        if diag is None or "junction_source_scores" not in diag:
            continue
        accum = rank_accum[layer_idx]
        source_scores = diag["junction_source_scores"].detach().float()
        for row in range(source_scores.shape[0]):
            q = min(int(queries[row].item()), source_scores.shape[1] - 1)
            if q < 0:
                continue
            scores = source_scores[row, q, : q + 1]
            useful = rel[row][rel[row] >= 0].clamp(max=q)
            distractors = dist[row][dist[row] >= 0].clamp(max=q)
            auc = _auc_for_positions(scores, useful, distractors)
            if auc is not None:
                accum["junction_auc_sum"] += auc
                accum["junction_auc_count"] += 1.0
            for k in (1, 3):
                recall = _topk_position_recall(scores, useful, k)
                if recall is not None:
                    accum[f"junction_top{k}_position_recall_sum"] += recall
                    accum[f"junction_top{k}_position_recall_count"] += 1.0
            if useful.numel() > 0:
                useful_scores = scores[useful]
                accum["junction_useful_concentration_sum"] += float(useful_scores.mean().cpu())
                accum["junction_useful_mass_sum"] += float(useful_scores.sum().cpu())
                accum["junction_useful_concentration_count"] += 1.0
            if distractors.numel() > 0:
                accum["junction_distractor_concentration_sum"] += float(scores[distractors].mean().cpu())
                accum["junction_distractor_concentration_count"] += 1.0


def _finish_evidence(rank_accum: List[Dict[str, float]]) -> Dict[str, List[Optional[float]]]:
    out: Dict[str, List[Optional[float]]] = {
        "useful_vs_distractor_avenue_auc": [],
        "useful_vs_distractor_junction_auc": [],
        "useful_evidence_avenue_concentration": [],
        "useful_evidence_junction_concentration": [],
        "useful_evidence_junction_mass": [],
        "distractor_avenue_concentration": [],
        "distractor_junction_concentration": [],
        "query_useful_gate_cosine": [],
    }
    for k in (1, 3):
        out[f"useful_top{k}_position_recall"] = []
        out[f"useful_top{k}_avenue_recall"] = []
        out[f"useful_top{k}_junction_recall"] = []
    for accum in rank_accum:
        ...
        jc = accum.get("junction_auc_count", 0.0)
        out["useful_vs_distractor_junction_auc"].append(accum["junction_auc_sum"] / jc if jc else None)
        ...
        juc = accum.get("junction_useful_concentration_count", 0.0)
        out["useful_evidence_junction_concentration"].append(
            accum["junction_useful_concentration_sum"] / juc if juc else None
        )
        out["useful_evidence_junction_mass"].append(accum["junction_useful_mass_sum"] / juc if juc else None)
        out["distractor_avenue_concentration"].append(accum["distractor_concentration_sum"] / dc if dc else None)
        jdc = accum.get("junction_distractor_concentration_count", 0.0)
        out["distractor_junction_concentration"].append(
            accum["junction_distractor_concentration_sum"] / jdc if jdc else None
        )
        ...
        for k in (1, 3):
            ...
            jpc = accum.get(f"junction_top{k}_position_recall_count", 0.0)
            out[f"useful_top{k}_junction_recall"].append(
                accum[f"junction_top{k}_position_recall_sum"] / jpc if jpc else None
            )
    return out
    ...
    for profile_idx, profile in enumerate(profiles):
        ...
        while remaining > 0:
            ...
            if collect_diagnostics:
                ...
                _update_junction_evidence_diagnostics(rank_accum, out.get("junction_attn"), batch)
...
def evaluate_avenue_controls(
    ...
    *,
    ...
    junction_controls: Dict[str, Dict[str, Any]] = {}
    if bool(getattr(model, "has_junctions", False)):
        point_controls = [
            f"junction_point_zero_{idx}"
            for idx in range(int(config.get("model", {}).get("n_layers", 4)))
        ]
        for control in list(JUNCTION_CONTROLS) + point_controls:
            metrics = evaluate_model(
                model,
                config,
                device,
    ...
    for idx in range(num_avenues):
        ...
        if num_avenues > 1:
            ...
            leave_metrics = evaluate_model(
                model,
                config,
                device,
                seed_offset=seed_offset,
                profiles=[profile],
                control=leave_control,
                collect_diagnostics=False,
            )
    ...
    return {
        ...
        "examples": examples,
        "controls": controls,
        "junction_controls": junction_controls,
...
def write_plots(runs: Sequence[Dict[str, Any]], plots_dir: Path) -> List[str]:
    ...
    try:
        ...
        for metric, filename, title in (
            ...
            ("junction_attention_entropy", "junction_entropy_by_layer.png", "Junction Attention Entropy by Layer"),
            ("junction_token_utilization", "junction_utilization_by_layer.png", "Junction Utilization by Layer"),
        ):
        ...
        _plot_bar(plots_dir / "leave_one_avenue_drop.png", list(loo.keys()), list(loo.values()), "Mean Leave-One-Avenue-Out Drop", "accuracy drop")
        ...
        _plot_bar(plots_dir / "full_vs_max_single_avenue.png", list(single_gap.keys()), list(single_gap.values()), "Full Minus Max Single Avenue", "accuracy gap")
        ...
        _plot_bar(plots_dir / "avenue_control_drops.png", list(avenue_drops.keys()), list(avenue_drops.values()), "Mean Avenue Control Drop", "accuracy drop")
        written.append(str(plots_dir / "avenue_control_drops.png"))
        junction_drops = {
            label: float(_mean(item.get("drop") for item in run.get("avenue_controls", {}).get("junction_controls", {}).values()) or 0.0)
            for label, run in zip(labels, runs)
        }
        _plot_bar(plots_dir / "junction_control_drops.png", list(junction_drops.keys()), list(junction_drops.values()), "Mean Junction Control Drop", "accuracy drop")
        written.append(str(plots_dir / "junction_control_drops.png"))
        _plot_bar(
...
def _conclusion(runs: Sequence[Dict[str, Any]]) -> str:
    ...
    if any(name not in by_variant for name in required):
        return "inconclusive: not all required E9 scout variants have completed runs"
    ...
    for name, run in by_variant.items():
        if name in {"baseline_transformer", "param_matched_baseline", "single_avenue_reference", "e8_best_reference"}:
            continue
        acc = float(run.get("eval", {}).get("accuracy", 0.0))
        controls = run.get("avenue_controls", {})
        avenue_control_drop = _mean(item.get("drop") for item in controls.get("controls", {}).values()) or 0.0
        junction_control_drop = _mean(item.get("drop") for item in controls.get("junction_controls", {}).values()) or 0.0
        mean_leave_drop = _mean(item.get("drop") for item in controls.get("leave_one_avenue_out", {}).values()) or 0.0
        ...
        junction_util = _mean(run.get("eval", {}).get("layers", {}).get("junction_token_utilization", []))
        if (
            acc > baseline
            and acc > param
            and avenue_control_drop > 0.02
            and junction_control_drop > 0.02
            and mean_leave_drop > 0.0
            ...
            and diversity > 0.05
            and junction_util is not None
            and junction_util > 0.2
        ):
    ...
    junction_runs = [run for run in runs if str(run.get("variant", "")).startswith("junction_") or str(run.get("variant")) in {"forced_junction_output", "scout_best_custom"}]
    if junction_runs and all(float(run.get("eval", {}).get("accuracy", 0.0)) <= baseline for run in junction_runs):
        return "not promising: all junction variants underperformed the baseline transformer, and junction controls did not reduce accuracy"
    best_avenue = max(
...
def write_report(
    ...
    *,
    ...
    for run in runs:
        layers = run.get("eval", {}).get("layers", {})
        controls = run.get("avenue_controls", {})
        avenue_drop = _mean(item.get("drop") for item in controls.get("controls", {}).values()) if controls else None
        junction_drop = _mean(item.get("drop") for item in controls.get("junction_controls", {}).values()) if controls else None
        leave_drop = _mean(item.get("drop") for item in controls.get("leave_one_avenue_out", {}).values()) if controls else None
        long_acc = _scenario_acc(run, "seq_len_256")
        lines.append(
            "| {variant} | {seed} | {params} | {acc} | {ind} | {long} | {high} | {ow} | {entropy} | {div} | {jutil} | {jdiv} | {gap} | {leave} | {adrop} | {jdrop} | {gpu} | {traintps} | {evaltps} |".format(
                ...
                jutil=_fmt(_mean(layers.get("junction_token_utilization", []))),
                jdiv=_fmt(_mean(layers.get("junction_diversity", []))),
                gap=_fmt(controls.get("full_minus_max_single_avenue") if controls else None),
                leave=_fmt(leave_drop),
                adrop=_fmt(avenue_drop),
                jdrop=_fmt(junction_drop),
    ...
    lines.extend(["", "## Junction Diagnostics", ""])
    for run in runs:
        layers = run.get("eval", {}).get("layers", {})
        controls = run.get("avenue_controls", {})
        lines.append(
            f"- `{run.get('variant')}` seed `{run.get('seed')}` "
            f"entropy={layers.get('junction_attention_entropy')} "
            f"avenue_to_junction_entropy={layers.get('avenue_to_junction_attention_entropy')} "
            f"junction_to_avenue_entropy={layers.get('junction_to_avenue_attention_entropy')} "
            f"utilization={layers.get('junction_token_utilization')} "
            f"norm={layers.get('junction_norm')} "
            f"diversity={layers.get('junction_diversity')} "
            f"contribution_norm={layers.get('junction_contribution_norm')} "
            f"controls={controls.get('junction_controls') if controls else 'NA'}"
        )
    ...
    for run in runs:
        ...
        if controls:
            lines.append(
                f"- `{run.get('variant')}` seed `{run.get('seed')}` "
                f"avenue_controls={controls.get('controls')} "
                f"junction_controls={controls.get('junction_controls')}"
            )
        else:
            lines.append(f"- `{run.get('variant')}` seed `{run.get('seed')}`: not applicable")
    ...
    for run in runs:
        ...
        lines.append(
            f"- `{run.get('variant')}` seed `{run.get('seed')}` "
            f"avenue_auc={layers.get('useful_vs_distractor_avenue_auc')} "
            f"junction_auc={layers.get('useful_vs_distractor_junction_auc')} "
            f"top1_pos={layers.get('useful_top1_position_recall')} "
            f"top3_pos={layers.get('useful_top3_position_recall')} "
            f"top1_junction={layers.get('useful_top1_junction_recall')} "
            f"top3_junction={layers.get('useful_top3_junction_recall')} "
            f"top1_avenue={layers.get('useful_top1_avenue_recall')} "
            f"top3_avenue={layers.get('useful_top3_avenue_recall')} "
            f"useful_avenue_concentration={layers.get('useful_evidence_avenue_concentration')} "
            f"useful_junction_concentration={layers.get('useful_evidence_junction_concentration')} "
            f"useful_junction_mass={layers.get('useful_evidence_junction_mass')} "
            f"distractor_avenue_concentration={layers.get('distractor_avenue_concentration')} "
            f"distractor_junction_concentration={layers.get('distractor_junction_concentration')} "
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluation/report utilities for 21EYES E9")
    parser.add_argument("--report", action="store_true")
    parser.add_argument("--results-dir", type=Path, default=None)
    parser.add_argument("--plots-dir", type=Path, default=None)
    parser.add_argument("--report-path", type=Path, default=None)
    parser.add_argument("--title", type=str, default="Experiment 9 Report")
    ...
    if results_dir is not None and not results_dir.is_absolute():
        results_dir = E9_ROOT / results_dir
    if plots_dir is not None and not plots_dir.is_absolute():
        plots_dir = E9_ROOT / plots_dir
    if report_path is not None and not report_path.is_absolute():
        report_path = E9_ROOT / report_path
    path = write_report(E9_ROOT, results_dir=results_dir, plots_dir=plots_dir, report_path=report_path, title=args.title)
    print(f"Wrote {path}")

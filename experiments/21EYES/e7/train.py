...
from eval import append_jsonl, evaluate_model, evaluate_slot_controls, save_json, write_report
...
def _resolve_batch_size(value: Any, *, device: torch.device, train: bool, has_slots: bool) -> int:
    if value is None or str(value).lower() == "auto":
        if device.type == "cuda":
            if has_slots:
                return 24 if train else 16
            return 64 if train else 32
        return 4 if has_slots else 8
    return int(value)
...
def _loss_for_batch(model: torch.nn.Module, batch, device: torch.device, amp_enabled: bool, config: Dict[str, Any]):
    ...
    with _autocast(device, amp_enabled):
        ...
        align_loss = aux.get("slot_alignment_loss", task_loss.new_zeros(()))
        diversity_loss = aux.get("slot_diversity_loss", task_loss.new_zeros(()))
        mirror_loss = aux.get("mirror_loss", task_loss.new_zeros(()))
        lambda_align = float(loss_cfg.get("lambda_align", 0.0))
        lambda_diversity = float(loss_cfg.get("lambda_diversity", 0.0))
        lambda_mirror = float(loss_cfg.get("lambda_mirror", 0.0))
        if variant not in {"slot_alignment_loss", "predictive_slot_residual"}:
            lambda_align = 0.0
        if variant != "mirrored_slot_attention":
            lambda_mirror = 0.0
        ...
        loss = (
            ...
            + lambda_mirror * mirror_loss
        )
    loss_parts = {
        "task_loss": task_loss.detach(),
        "slot_alignment_loss": align_loss.detach(),
        "slot_diversity_loss": diversity_loss.detach(),
        "mirror_loss": mirror_loss.detach(),
        "total_loss": loss.detach(),
    }
    return loss, out, loss_parts
...
def run_training(config_path: Path, args: argparse.Namespace) -> Path:
    ...
    has_slots = variant not in {"baseline_transformer", "param_matched_baseline"}
    batch_size = _resolve_batch_size(data_cfg.get("batch_size", "auto"), device=device, train=True, has_slots=has_slots)
    eval_batch_size = _resolve_batch_size(
        data_cfg.get("eval_batch_size", "auto"),
        device=device,
        train=False,
        has_slots=has_slots,
    )
    ...
    output_root = Path(args.output_root).resolve() if args.output_root is not None else E7_ROOT / "results"
    ...
    for step in range(1, max_steps + 1):
        ...
        loss, _out, loss_parts = _loss_for_batch(model, batch, device, amp_enabled, config)
        ...
        if step % log_every == 0:
            ...
            append_jsonl(
                metrics_path,
                {
                    ...
                    "slot_alignment_loss": float(loss_parts["slot_alignment_loss"].float().cpu()),
                    "slot_diversity_loss": float(loss_parts["slot_diversity_loss"].float().cpu()),
                    "mirror_loss": float(loss_parts["mirror_loss"].float().cpu()),
    ...
    slot_controls = evaluate_slot_controls(model, config, device, seed_offset=700_000)
    peak_memory_mb = None
    if device.type == "cuda":
    ...
    if output_root == E7_ROOT / "results":
        write_report(E7_ROOT)
    print(f"Wrote {run_dir / 'final_metrics.json'}")
    return run_dir


def main() -> None:
    parser = argparse.ArgumentParser(description="Train a 21EYES Experiment 7 variant")
    ...
    if not config_path.is_absolute():
        config_path = E7_ROOT / config_path

"""Loss functions for the DIGIT Extrapolation E10 cooperative diffusion variant.

E10 replaces the split E9 stack with a shared diffusion-evidence manifold:
  - each reconstructed path emits order / boundary / support evidence
  - certainty, guard, confidence, outcome, and commitment read the same evidence
  - prototype geometry regularises those path states instead of replacing them
  - perturbation losses enforce downhill movement in that ordered evidence space
"""
...
class DIGITLoss(nn.Module):
    """Combined loss for the E10 cooperative diffusion-evidence scaffold."""
    ...
    def forward(
        ...
        pattern_margin_loss = (
            self._pattern_margin_loss(primitives, prim_targets)
            if float(getattr(self.config, "lambda_pattern_margin", 0.0)) != 0.0
            else zero
        )
        losses["pattern_margin"] = pattern_margin_loss
        ...
        path_evidence_target_loss = (
            self._path_evidence_target_loss(primitives, evidence_targets, prim_targets, executor_features)
            if evidence_targets is not None and float(getattr(self.config, "lambda_path_evidence_targets", 0.0)) != 0.0
            else zero
        )
        losses["path_evidence_targets"] = path_evidence_target_loss

        cooperative_head_target_loss = (
            self._cooperative_head_target_loss(primitives, evidence_targets, executor_features)
            if evidence_targets is not None and float(getattr(self.config, "lambda_cooperative_head_targets", 0.0)) != 0.0
            else zero
        )
        losses["cooperative_head_targets"] = cooperative_head_target_loss
        ...
        losses["total"] = (
            ...
            + float(getattr(self.config, "lambda_pattern_margin", 0.0)) * pattern_margin_loss
            + self.config.lambda_generation * generation_loss
            + self.config.lambda_evidence_targets * evidence_target_loss
            + float(getattr(self.config, "lambda_path_evidence_targets", 0.0)) * path_evidence_target_loss
            + float(getattr(self.config, "lambda_cooperative_head_targets", 0.0)) * cooperative_head_target_loss
    ...
    def _pattern_margin_loss(
        self,
        primitives: PrimitiveOutput,
        prim_targets: torch.Tensor,
    ) -> torch.Tensor:
        logits = primitives.attention_pattern_logits
        margin = float(getattr(self.config, "trace_pattern_logit_margin", 0.15))
        focused_logit = logits[:, 0]
        mixed_logit = logits[:, 1]
        diffuse_logit = logits[:, 2]
        pattern_targets = prim_targets[:, 1]

        focused_mask = (pattern_targets == 0).float()
        mixed_mask = (pattern_targets == 1).float()
        diffuse_mask = (pattern_targets == 2).float()

        focused_loss = (
            focused_mask * torch.relu(margin - (focused_logit - mixed_logit))
        ).sum() / focused_mask.sum().clamp_min(1.0)
        diffuse_loss = (
            diffuse_mask * torch.relu(margin - (diffuse_logit - mixed_logit))
        ).sum() / diffuse_mask.sum().clamp_min(1.0)
        mixed_loss = (
            mixed_mask
            * (
                torch.relu(margin - (mixed_logit - focused_logit))
                + torch.relu(margin - (mixed_logit - diffuse_logit))
            )
        ).sum() / mixed_mask.sum().clamp_min(1.0)

        return 0.30 * focused_loss + 0.40 * mixed_loss + 0.55 * diffuse_loss
    ...
    @staticmethod
    def _support_target_from_evidence_targets(
        ...
        effective_support = executor_feature_column(executor_features, "attention_effective_support")
        top2_mass = executor_feature_column(executor_features, "attention_top2_mass")
        variation_ratio = executor_feature_column(executor_features, "variation_ratio")
        quality_proxy = torch.clamp(
            0.22 * agreement
            + 0.18 * prob_margin
            + 0.18 * max_attention
            + 0.20 * effective_support
            + 0.12 * top2_mass
            + 0.10 * (1.0 - variation_ratio),
            min=0.0,
            max=1.0,
        )
        return torch.clamp(0.60 * evidence_targets[:, 1] + 0.40 * quality_proxy, min=0.0, max=1.0)

    def _path_evidence_target_loss(
        self,
        primitives: PrimitiveOutput,
        evidence_targets: torch.Tensor,
        prim_targets: torch.Tensor,
        ...
        support_target = self._support_target_from_evidence_targets(evidence_targets, executor_features)
        boundary_target = torch.clamp(
            0.65 * evidence_targets[:, 4] + 0.35 * (prim_targets[:, 3] == 1).float(),
            min=0.0,
            max=1.0,
        )
        order_target = torch.clamp(evidence_targets[:, 2] - evidence_targets[:, 3], min=-1.0, max=1.0)
        outcome_target = torch.stack(
            [
                evidence_targets[:, 2],
                evidence_targets[:, 4],
                evidence_targets[:, 3],
        ...
        outcome_target = outcome_target / outcome_target.sum(dim=-1, keepdim=True).clamp_min(1e-6)

        support_mean = primitives.path_support_paths.mean(dim=0)
        boundary_mean = primitives.path_boundary_paths.mean(dim=0)
        order_mean = primitives.path_order_score_paths.mean(dim=0)
        mean_outcome = primitives.path_outcome_prob_paths.mean(dim=0).clamp_min(1e-6)
        mean_outcome = mean_outcome / mean_outcome.sum(dim=-1, keepdim=True).clamp_min(1e-6)

        support_loss = F.smooth_l1_loss(support_mean, support_target)
        boundary_loss = F.smooth_l1_loss(boundary_mean, boundary_target)
        order_loss = F.smooth_l1_loss(order_mean, order_target)
        outcome_loss = F.kl_div(mean_outcome.log(), outcome_target, reduction="batchmean")

        expanded_support = support_target.unsqueeze(0).expand_as(primitives.path_support_paths)
        expanded_boundary = boundary_target.unsqueeze(0).expand_as(primitives.path_boundary_paths)
        expanded_order = order_target.unsqueeze(0).expand_as(primitives.path_order_score_paths)
        per_path_support_loss = F.smooth_l1_loss(primitives.path_support_paths, expanded_support)
        per_path_boundary_loss = F.smooth_l1_loss(primitives.path_boundary_paths, expanded_boundary)
        per_path_order_loss = F.smooth_l1_loss(primitives.path_order_score_paths, expanded_order)
        ...
        return (
            ...
            + 0.20 * order_loss
            + 0.35 * outcome_loss
            + 0.20 * per_path_support_loss
            + 0.15 * per_path_boundary_loss
            + 0.10 * per_path_order_loss
        )

    def _cooperative_head_target_loss(
        self,
        primitives: PrimitiveOutput,
        evidence_targets: torch.Tensor,
        executor_features: torch.Tensor,
        ...
        stability_head_loss = F.smooth_l1_loss(primitives.stability_cert, stability_target)
        support_head_loss = F.smooth_l1_loss(primitives.support_cert, support_target)
        guard_head_loss = F.smooth_l1_loss(primitives.success_guard, guard_target)
        certainty_margin_loss = torch.relu(primitives.support_cert - primitives.stability_cert - 0.10).mean()
        return stability_head_loss + support_head_loss + guard_head_loss + 0.20 * certainty_margin_loss
    ...
    def _family_supervision_loss(
        ...
        family_class_weights = family_support.new_tensor(
            [
                float(getattr(self.config, "anchor_success_family_weight", 1.0)),
                float(getattr(self.config, "anchor_failure_family_weight", 1.0)),
                1.0,
            ]
        )
        sample_weights = family_class_weights.gather(0, family_targets)
        normalised_sample_weights = sample_weights / sample_weights.mean().clamp_min(1e-6)
        ce_loss = F.nll_loss(
            family_log_probs,
            family_targets,
            weight=family_class_weights,
            reduction="mean",
        )
        ...
        margin_loss = (
            normalised_sample_weights
            * (decisive_mask * decisive_margin + (1.0 - decisive_mask) * boundary_margin)
        ).mean()
        top_family_mismatch = (
            normalised_sample_weights
            * (primitives.top_family_id.long() != family_targets).float()
        ).mean()

        pred_usage = family_support.mean(dim=0).clamp_min(1e-6)
        pred_usage = pred_usage / pred_usage.sum().clamp_min(1e-6)
        target_usage = F.one_hot(family_targets, num_classes=family_support.size(-1)).float().mean(dim=0)
        target_usage = target_usage / target_usage.sum().clamp_min(1e-6)
        activity_target = float(getattr(self.config, "anchor_family_activity_ratio", 0.0)) * target_usage[:2]
        activity_weights = family_class_weights[:2] / family_class_weights[:2].mean().clamp_min(1e-6)
        activity_loss = (
            activity_weights * torch.relu(activity_target - pred_usage[:2])
        ).sum()
        failure_mask = (family_targets == 1).float()
        failure_boundary_margin = torch.relu(
            float(getattr(self.config, "anchor_failure_boundary_margin", 0.0))
            - (primitives.failure_family_support - primitives.boundary_family_support)
        )
        failure_boundary_margin_loss = (
            normalised_sample_weights
            * failure_mask
            * failure_boundary_margin
        ).mean()
        if hasattr(primitives, "prototype_family_distribution_paths") and hasattr(
            ...
            path_margin_loss = (
                normalised_sample_weights[:, None]
                * (
            ...
            path_mismatch = (
                normalised_sample_weights[:, None]
                * (path_top_family != family_targets[:, None]).float()
            ).mean()
            path_pred_usage = path_family_probs.mean(dim=(0, 1)).clamp_min(1e-6)
            path_pred_usage = path_pred_usage / path_pred_usage.sum().clamp_min(1e-6)
            path_activity_loss = (
                activity_weights * torch.relu(activity_target - path_pred_usage[:2])
            ).sum()
            path_failure_boundary_margin = torch.relu(
                float(getattr(self.config, "anchor_failure_boundary_margin", 0.0))
                - (path_support[:, :, 1] - path_support[:, :, 2])
            )
            path_failure_boundary_margin_loss = (
                normalised_sample_weights[:, None]
                * failure_mask[:, None]
                * path_failure_boundary_margin
            ).mean()
        else:
            ...
            path_activity_loss = family_support.new_tensor(0.0)
            path_failure_boundary_margin_loss = family_support.new_tensor(0.0)

        return (
            ce_loss
            + float(getattr(self.config, "anchor_family_margin_weight", 0.0)) * margin_loss
            + float(getattr(self.config, "anchor_family_activity_weight", 0.0))
            * (activity_loss + 0.5 * path_activity_loss)
            + float(getattr(self.config, "anchor_failure_boundary_margin_weight", 0.0))
            * (failure_boundary_margin_loss + 0.5 * path_failure_boundary_margin_loss)
    ...
    def _consistency_loss(self, primitives: PrimitiveOutput) -> torch.Tensor:
        ...
        cert_alignment = (
            ...
            + torch.relu(success - primitives.success_guard + 0.02).mean()
            + torch.relu(commitment - primitives.success_guard + 0.02).mean()
        ) / 7.0

...
from experiments.DIGIT.Extrapolation.e10.e10_falsified.extrapolation.data.ground_truth import (
    ATTENTION_PATTERN_LABELS,
    CONFIDENCE_LABELS,
    OUTCOME_LABELS,
    TRAJECTORY_SHAPE_LABELS,
)
from experiments.DIGIT.Extrapolation.e10.e10_falsified.extrapolation.data.trace_dataset import TraceEntropyDataset, load_trace_records, trace_collate_fn
from experiments.DIGIT.Extrapolation.e10.e10_falsified.extrapolation.data.vocabulary import Vocabulary
from experiments.DIGIT.Extrapolation.e10.e10_falsified.extrapolation.losses import DIGITLoss
from experiments.DIGIT.Extrapolation.e10.e10_falsified.extrapolation.models.digit import DIGITModel
from experiments.DIGIT.Extrapolation.e10.e10_falsified.extrapolation.trace_pipeline import (
    TRACE_LABEL_VERSION,
    ensure_trace_corpus,
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            if hasattr(out["primitives"], "stability_cert"):
                ...
                all_evidence["effective_certainty"].append(out["primitives"].effective_certainty.cpu())
                ...
                all_evidence["prototype_total_support"].append(out["primitives"].prototype_total_support.cpu())
                all_evidence["prototype_family_margin"].append(out["primitives"].prototype_family_margin.cpu())
                all_evidence["prototype_overlap"].append(out["primitives"].prototype_overlap.cpu())
                all_evidence["prototype_family_switch_rate"].append(
                    out["primitives"].prototype_family_switch_rate.cpu()
                )
                all_evidence["prototype_anchor_switch_rate"].append(
                    out["primitives"].prototype_anchor_switch_rate.cpu()
                )
                all_evidence["prototype_nearest_anchor_distance"].append(
                    out["primitives"].prototype_nearest_anchor_distance.cpu()
                )
...
def _collapse_audit_from_bundle(
    ...
    if all(
        ...
        family_support = torch.stack(
            [
                evidence["success_family_support"].float().cpu(),
                evidence["failure_family_support"].float().cpu(),
                evidence["boundary_family_support"].float().cpu(),
            ],
            dim=-1,
        ).clamp_min(1e-6)
        family_support_probs = family_support / family_support.sum(dim=-1, keepdim=True).clamp_min(1e-6)
        family_usage_shares = {
            family_name: float(family_support_probs[:, idx].mean().item())
    ...
    if "top_family_id" in evidence:
        ...
        if not family_usage_shares:
            family_usage_shares = dict(family_usage_hard_shares)
    ...
    return {
        ...
        "family_usage_hard_shares": family_usage_hard_shares,
...
def _success_guard_target_from_evidence_targets(evidence_targets: torch.Tensor) -> torch.Tensor:
    ...
    return torch.minimum(success_target, 0.90 * guard_anchor + 0.10 * success_target)
...
def _diagnostic_examples_from_bundle(
    ...
    def build_row(idx: int, category: str, score: float, reason: str) -> dict[str, Any]:
        ...
        return {
            ...
            "certainty_score": float(evidence.get("certainty_score", torch.zeros_like(success_cert))[idx].item()),
            "fragility_risk": float(evidence.get("fragility_risk", torch.zeros_like(success_cert))[idx].item()),
            "effective_certainty": float(
                evidence.get("effective_certainty", torch.zeros_like(success_cert))[idx].item()
            ),
            ...
            "prototype_total_support": float(
                evidence.get("prototype_total_support", torch.zeros_like(success_cert))[idx].item()
            ),
            "prototype_family_switch_rate": float(
                evidence.get("prototype_family_switch_rate", torch.zeros_like(success_cert))[idx].item()
            ),
            "prototype_anchor_switch_rate": float(
                evidence.get("prototype_anchor_switch_rate", torch.zeros_like(success_cert))[idx].item()
            ),
            "prototype_nearest_anchor_distance": float(
                evidence.get("prototype_nearest_anchor_distance", torch.zeros_like(success_cert))[idx].item()
            ),
...
def _anchor_audit_from_bundle(bundle: dict[str, Any], anchor_domination_threshold: float = 0.95) -> dict[str, Any]:
    ...
    family_support_tensor = torch.stack(
        [
            evidence["success_family_support"].float().cpu(),
            evidence["failure_family_support"].float().cpu(),
            evidence["boundary_family_support"].float().cpu(),
    ...
    family_support_probs = family_support_tensor / family_support_tensor.sum(dim=-1, keepdim=True).clamp_min(1e-6)
    family_usage_shares = {
        family_name: float(family_support_probs[:, idx].mean().item())
        for idx, family_name in enumerate(family_names)
    }
...
def _anchor_geometry_from_model(model: DIGITModel) -> dict[str, Any]:
    del model
    return {"variant": "cooperative_diffusion_e10"}
...
def _commitment_audit_from_bundle(bundle: dict[str, Any]) -> dict[str, Any]:
    ...
    raw_conflict = evidence["anchor_conflict"].cpu()
    trust_conflict = evidence.get("trust_conflict", evidence["anchor_conflict"]).cpu()
    family_margin = evidence.get("prototype_family_margin", torch.zeros_like(trust_conflict)).cpu()
    prototype_overlap = evidence.get("prototype_overlap", torch.zeros_like(trust_conflict)).cpu()
    commitment = evidence["commitment_depth"].cpu()
    outcome_probs = probs["outcome"].cpu()
    predicted_outcome = predictions[:, 3].cpu()
    predicted_success = predicted_outcome == 0
    high_conflict = (trust_conflict >= 0.78) | ((family_margin < 0.08) & (prototype_overlap > 0.55)) | (raw_conflict >= 0.92)
...
def _monotonicity_offender_examples(
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            base_effective_certainty = base_out["primitives"].effective_certainty.cpu()
            ...
            base_prototype_total_support = base_out["primitives"].prototype_total_support.cpu()
            base_prototype_family_margin = base_out["primitives"].prototype_family_margin.cpu()
            base_prototype_overlap = base_out["primitives"].prototype_overlap.cpu()
            base_prototype_family_switch_rate = base_out["primitives"].prototype_family_switch_rate.cpu()
            base_prototype_switch_rate = base_out["primitives"].prototype_anchor_switch_rate.cpu()
            base_prototype_nearest_anchor_distance = base_out["primitives"].prototype_nearest_anchor_distance.cpu()
            ...
            for kind in perturbation_kinds:
                ...
                perturbed_effective_certainty = perturbed_out["primitives"].effective_certainty.cpu()
                ...
                perturbed_prototype_total_support = perturbed_out["primitives"].prototype_total_support.cpu()
                perturbed_prototype_family_margin = perturbed_out["primitives"].prototype_family_margin.cpu()
                perturbed_prototype_overlap = perturbed_out["primitives"].prototype_overlap.cpu()
                perturbed_prototype_family_switch_rate = (
                    perturbed_out["primitives"].prototype_family_switch_rate.cpu()
                )
                perturbed_prototype_switch_rate = perturbed_out["primitives"].prototype_anchor_switch_rate.cpu()
                perturbed_prototype_nearest_anchor_distance = (
                    perturbed_out["primitives"].prototype_nearest_anchor_distance.cpu()
                )
                    ...
                    rows.append(
                        {
                            ...
                            "base_effective_certainty": float(base_effective_certainty[idx].item()),
                            "perturbed_effective_certainty": float(perturbed_effective_certainty[idx].item()),
                            ...
                            "base_prototype_total_support": float(base_prototype_total_support[idx].item()),
                            "perturbed_prototype_total_support": float(perturbed_prototype_total_support[idx].item()),
                            ...
                            "base_prototype_overlap": float(base_prototype_overlap[idx].item()),
                            "perturbed_prototype_overlap": float(perturbed_prototype_overlap[idx].item()),
                            "base_prototype_family_switch_rate": float(
                                base_prototype_family_switch_rate[idx].item()
                            ),
                            "perturbed_prototype_family_switch_rate": float(
                                perturbed_prototype_family_switch_rate[idx].item()
                            ),
                            ...
                            "base_prototype_nearest_anchor_distance": float(
                                base_prototype_nearest_anchor_distance[idx].item()
                            ),
                            "perturbed_prototype_nearest_anchor_distance": float(
                                perturbed_prototype_nearest_anchor_distance[idx].item()
                            ),
...
def _success_rate_excess(metrics: dict[str, float | list[list[int]]], config: Config) -> float:
    predicted_success_rate = _metric_or_default(metrics, "predicted_success_rate", 0.0)
    return max(
        predicted_success_rate - float(config.trace_selection_success_rate_max),
        0.0,
    )


def _stage1_balanced_score(metrics: dict[str, float | list[list[int]]], config: Config) -> float:
    return (
        3.0 * _metric_or_default(metrics, "outcome_macro_f1", 0.0)
        + 1.0 * _metric_or_default(metrics, "failure_recall", 0.0)
        + 1.0 * _metric_or_default(metrics, "pattern_macro_f1", 0.0)
        + 0.75 * _metric_or_default(metrics, "pattern_diffuse_f1", 0.0)
        + 2.0 * _metric_or_default(metrics, "trajectory_macro_f1", 0.0)
        + 1.5 * _metric_or_default(metrics, "confidence_macro_f1", 0.0)
        + 1.5 * _metric_or_default(metrics, "joint_acc", 0.0)
        - 0.75 * _metric_or_default(metrics, "commitment_monotonicity_violation_rate", 0.0)
        - 0.25 * _success_rate_excess(metrics, config)
        - 0.10 * _metric_or_default(metrics, "success_aurc", 0.0)
    )
...
def _is_better_stage1(
    ...
    if abs(candidate_outcome - best_outcome) <= tolerance:
        ...
        if abs(candidate_failure - best_failure) <= tolerance:
            candidate_score = _stage1_balanced_score(metrics, config)
            best_score = _stage1_balanced_score(best_metrics, config)
            if candidate_score > best_score + tolerance:
                return True
            if abs(candidate_score - best_score) <= tolerance:
                candidate_pattern = _metric_or_default(metrics, "pattern_macro_f1", 0.0)
                best_pattern = _metric_or_default(best_metrics, "pattern_macro_f1", 0.0)
                if candidate_pattern > best_pattern + tolerance:
                    return True
                if abs(candidate_pattern - best_pattern) <= tolerance:
                    return (
                        _metric_or_default(metrics, "pattern_diffuse_f1", 0.0)
                        > _metric_or_default(best_metrics, "pattern_diffuse_f1", 0.0) + 1e-8
                    )
    return False
...
def _stage1_candidate_retention_key(candidate: dict[str, Any]) -> tuple[float, ...]:
    ...
    return (
        ...
        _stage1_balanced_score(metrics, config),
        _metric_or_default(metrics, "outcome_macro_f1", -1e9),
        _metric_or_default(metrics, "failure_recall", -1e9),
        _metric_or_default(metrics, "pattern_macro_f1", -1e9),
        _metric_or_default(metrics, "pattern_diffuse_f1", -1e9),
        ...
        -_metric_or_default(metrics, "commitment_monotonicity_violation_rate", 1e9),
        -_success_rate_excess(metrics, config),
    )
...
def _select_stage1_candidate(
    ...
    if non_collapsed:
        ...
        best_pattern = max(_metric_or_default(candidate["metrics"], "pattern_macro_f1", -1e9) for candidate in pool)
        best_diffuse = max(_metric_or_default(candidate["metrics"], "pattern_diffuse_f1", -1e9) for candidate in pool)
        ...
        pattern_floor = best_pattern - float(config.trace_stage1_frontier_pattern_slack)
        diffuse_floor = best_diffuse - float(config.trace_stage1_frontier_diffuse_slack)
        frontier = [
            candidate
            ...
            and _metric_or_default(candidate["metrics"], "pattern_macro_f1", -1e9) >= pattern_floor
            and _metric_or_default(candidate["metrics"], "pattern_diffuse_f1", -1e9) >= diffuse_floor
        ]
        pool = frontier or pool
        return max(
            pool,
            key=lambda candidate: (
                _stage1_balanced_score(candidate["metrics"], config),
                ...
                _metric_or_default(candidate["metrics"], "pattern_macro_f1", -1e9),
                _metric_or_default(candidate["metrics"], "pattern_diffuse_f1", -1e9),
                _metric_or_default(candidate["metrics"], "failure_recall", -1e9),
                -_metric_or_default(candidate["metrics"], "commitment_monotonicity_violation_rate", 1e9),
                -_success_rate_excess(candidate["metrics"], config),
...
def _vocab_payload(vocab: Vocabulary) -> dict[str, Any]:
    return {
        "class_name": vocab.__class__.__name__,
        "tokens": list(vocab.tokens),
        "token_to_id": dict(vocab.token_to_id),
        "id_to_token": {str(key): value for key, value in vocab.id_to_token.items()},
        "pad_idx": int(vocab.pad_idx),
        "bos_idx": int(vocab.bos_idx),
        "eos_idx": int(vocab.eos_idx),
        "unk_idx": int(vocab.unk_idx),
    }
...
def _save_checkpoint_bundle(
    ...
    *,
    ...
    vocab_payload = _vocab_payload(vocab)
    best_payload = {
        **metadata,
        "checkpoint_kind": "best",
        "model_state_dict": _state_dict_to_cpu(best_state),
    ...
    final_payload = {
        **metadata,
        ...
        "model_state_dict": _state_dict_to_cpu(final_state),
        "config": config_payload,
        "vocab": vocab_payload,
    }
    ...
    write_json(checkpoint_dir / "vocab.json", vocab_payload)
    readme = (
        "E10 checkpoint bundle\n"
        f"Run: {metadata.get('run_name', 'unknown')}\n"
        "Selected checkpoint: best.pt\n"
        "Final checkpoint: final.pt\n"
        f"Selected stage1 epoch: {metadata.get('selected_stage1_epoch', 'unknown')}\n"
        f"Stage2 enabled: {metadata.get('stage2_enabled', False)}\n"
        f"Selected state source: {metadata.get('selected_state_source', 'unknown')}\n"
        f"Final state source: {metadata.get('final_state_source', 'unknown')}\n"
    )
    (checkpoint_dir / "README.txt").write_text(readme)
    return {
        "best": str(best_path),
        "final": str(final_path),
        "config": str(checkpoint_dir / "config.json"),
        "vocab": str(checkpoint_dir / "vocab.json"),
        "readme": str(checkpoint_dir / "README.txt"),
    }
...
def main() -> None:
    ...
    class_weights["outcome"] = torch.tensor(
        [
            config.trace_outcome_success_weight,
            config.trace_outcome_uncertain_weight,
            config.trace_outcome_failure_weight,
        ],
        dtype=torch.float32,
        device=device,
    )
    ...
    warmup_config.trace_guard_quality_weight = 0.0
    warmup_config.trace_success_prior_weight = 0.0
    ...
    print(
        ...
        "(retention: non-collapsed -> outcome_macro_f1 -> failure_recall -> pattern_macro_f1 -> pattern_diffuse_f1 -> lower commitment_violation -> lower success-rate excess -> trajectory_macro_f1 -> confidence_macro_f1 -> joint_acc; "
        "final selection: frontier(outcome/failure) -> pattern_macro_f1 -> pattern_diffuse_f1 -> lower commitment_violation -> lower success-rate excess -> trajectory_macro_f1 -> confidence_macro_f1 -> lower success_aurc -> joint_acc)"
    )

    base_stage1_epochs = int(config.trace_stage1_epochs)
    rescue_stage1_epochs = max(int(config.trace_stage1_rescue_epochs), 0)
    max_stage1_epochs = base_stage1_epochs + rescue_stage1_epochs
    warmup_epochs = min(config.trace_stage1_head_warmup_epochs, base_stage1_epochs)
    perturbation_kinds = [
        "lower_agreement",
        "lower_margin",
        "higher_entropy",
    ...
    for epoch in range(max_stage1_epochs):
        rescue_phase = epoch >= base_stage1_epochs
        if rescue_phase and stage1_candidate_bank:
            break
        if rescue_phase and epoch == base_stage1_epochs:
            print(
                "No eligible non-collapsed stage-1 checkpoint found in the base schedule; "
                "entering rescue extension with full primitive losses."
            )
        forced_barrier = (
            rescue_phase
            and bool(config.trace_stage1_rescue_force_barrier)
            and not stage1_candidate_bank
        )
        if forced_barrier and not barrier_active and barrier_activation_epoch is None:
            barrier_activation_epoch = int(epoch + 1)
            print(
                f"Barrier forced on for rescue extension at epoch {barrier_activation_epoch}."
            )
        model.train()
        epoch_loss = 0.0
        epoch_items = 0
        ...
        epoch_loss_terms = {key: 0.0 for key in tracked_loss_keys}
        criterion = stage1_criterion if rescue_phase else (
            warmup_criterion if epoch < warmup_epochs else stage1_criterion
        )
        current_stage1_path_scales = _runtime_path_scales(
            config,
            barrier_active=(barrier_active or forced_barrier),
        )
        phase_label = "Stage1 Rescue" if rescue_phase else "Stage1"
        phase_epoch = epoch + 1 - base_stage1_epochs if rescue_phase else epoch + 1
        phase_total = rescue_stage1_epochs if rescue_phase else base_stage1_epochs
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            for key in tracked_loss_keys:
                if key in losses:
                    epoch_loss_terms[key] += float(losses[key].item()) * batch_size
        ...
        val_commitment_monotonicity = _commitment_monotonicity_audit(
            model,
            val_loader,
            device,
            path_scales=current_stage1_path_scales,
        )
        val_metrics["commitment_monotonicity_violation_rate"] = float(
            val_commitment_monotonicity["commitment_monotonicity_violation_rate"]
        )
        val_metrics["success_prob_monotonicity_violation_rate"] = float(
            val_commitment_monotonicity["success_prob_monotonicity_violation_rate"]
        )
        ...
        print(
            f"{phase_label} Epoch {phase_epoch:02d}/{phase_total} | "
            ...
            f"val_commitment_violation={_metric_or_default(val_metrics, 'commitment_monotonicity_violation_rate', float('nan')):.3f} | "
        ...
        eligible_stage1_candidate = (
            not config.trace_require_noncollapsed_stage1_candidate
            or not candidate_collapsed
        )
        if eligible_stage1_candidate:
        ...
        if eligible_stage1_candidate and _is_better_stage1(
            val_metrics,
            best_stage1_metrics,
        ...
        epoch_history.append(
            {
                "stage": "stage1_rescue" if rescue_phase else "stage1",
                "epoch": int(epoch + 1),
                ...
                "val_commitment_monotonicity_violation_rate": float(
                    _metric_or_default(val_metrics, "commitment_monotonicity_violation_rate", float("nan"))
                ),
                ...
                "val_pattern_diffuse_f1": float(_metric_or_default(val_metrics, "pattern_diffuse_f1", float("nan"))),
                "val_pattern_diffuse_recall": float(_metric_or_default(val_metrics, "pattern_diffuse_recall", float("nan"))),
                ...
                **{
                    f"train_{key}": float(epoch_loss_terms[key] / max(epoch_items, 1))
                    for key in tracked_loss_keys
                },
                "val_collapse_audit": dict(val_collapse_audit),
    ...
    stage1_final_state = copy.deepcopy(model.state_dict())

    if best_stage1_collapse_audit.get("collapsed_model", False):
        message = "No non-collapsed stage-1 checkpoint satisfied the validation collapse audit."
        write_json(
            output_dir / "stage1_failure_debug.json",
            {
                "message": message,
                "selected_stage1_epoch": best_stage1_epoch,
                "best_stage1_metrics": best_stage1_metrics,
                "best_stage1_collapse_audit": best_stage1_collapse_audit,
                "best_stage1_path_scales": best_stage1_path_scales,
                "barrier_activation_epoch": barrier_activation_epoch,
                "epoch_history": epoch_history,
            },
        )
        if config.trace_require_noncollapsed_stage1_candidate:
    ...
    if not config.trace_force_stage1_only and config.trace_stage2_epochs > 0:
        ...
        stage2_final_state = copy.deepcopy(model.state_dict())
        ...
        stage2_final_state = None

    final_state = stage2_final_state if stage2_final_state is not None else stage1_final_state
    ...
    checkpoint_metadata = {
        ...
        "best_stage1_collapse_audit": best_stage1_collapse_audit,
        "best_stage1_path_scales": best_stage1_path_scales,
        "stage2_enabled": bool(not config.trace_force_stage1_only and config.trace_stage2_epochs > 0),
        "best_stage2_loss": float(best_stage2_loss),
        "primitive_predictions_preserved": bool(primitive_predictions_preserved),
        "selected_state_source": "best_stage2_state" if stage2_final_state is not None else "best_stage1_state",
        "final_state_source": "stage2_final_state" if stage2_final_state is not None else "stage1_final_state",
    }

# E31 Architecture Note

`e31` extends the `e30` witness-agreement experiment with v3-style latent path
stability. The goal is to make uncertainty signals cooperate: endpoint witness
agreement remains the primary decision geometry, while independent diffusion
trajectories add a compact stability check.
...
When `enable_path_uncertainty = true`, the diffusion sampler returns the full
reverse trajectory for two independent noise views:

```text
trajectory_a, trajectory_b: [T + 1, K, B, d_model]
```

A small `LatentPathEncoder` compresses each witness trajectory into a bottleneck
embedding. The per-example path disagreement is:

```text
P = 1 - cosine(path_embed_a, path_embed_b)
```

The path encoder is trained with a self-supervised two-view contrastive loss. It
does not receive outcome labels, and the path tensors are detached before the
path loss so this auxiliary objective does not reshape the diffusion model.

Default `e31` behavior is diagnostics-first:

```text
enable_path_uncertainty = true
use_path_disagreement_in_uncertainty = false
use_method_conflict_in_uncertainty = false
use_path_commitment_penalty = false
use_path_aware_witness_weights = false
```

This logs the path signal without changing the main decision rule.

## Cooperative Path/Witness Mode

For the active cooperative ablation, enable:

```bash
--enable-cooperative-path-uncertainty
```

This adds path disagreement and endpoint/path conflict into uncertainty:

```text
endpoint_confidence = abs(M) / (abs(M) + 1)
path_confidence = clamp(1 - 0.5 * P, 0, 1)
R = abs(endpoint_confidence - path_confidence)

U = softplus(alpha * D + beta * G + delta * P + rho * R - gamma * abs(M) + b)
C = abs(M) - lambda_commit * U - lambda_path * P
```

For the full coupling ablation, also enable:

```bash
--enable-path-aware-witness-weights
```

This lets path instability modulate witness trust:

```text
omega_k = softmax((coherence_k - eta * path_instability_k) / temperature)
```
...
- The path bottleneck is explicit, logged, and self-supervised.
- The default reader path remains unchanged unless cooperative flags are enabled.

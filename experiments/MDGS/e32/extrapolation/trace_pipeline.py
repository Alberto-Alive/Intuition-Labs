"""Local trace-cache helpers and simple baselines for E31."""
...
def _majority_label(values: np.ndarray) -> Any:
    ...
    if unique.size == 0:
        raise ValueError("Cannot compute a majority label from an empty target array.")
...
def evaluate_trace_baselines(train_records: list[Dict[str, Any]], test_records: list[Dict[str, Any]], config=None) -> Dict[str, Dict]:
    ...
    for target_name in ["trajectory_shape", "attention_pattern", "confidence", "outcome"]:
        train_values = y_train[target_name]
        test_values = y_test[target_name]
        majority_label = _majority_label(train_values)
        majority_predictions = np.full(test_values.shape, majority_label, dtype=test_values.dtype)
        unique_classes, class_counts = np.unique(train_values, return_counts=True)
        ...
        try:
            ...
            lr_model.fit(x_train_summary, train_values)
            lr_pred = lr_model.predict(x_test_summary)
            logistic[target_name] = {
                "accuracy": float(accuracy_score(test_values, lr_pred)),
                "macro_f1": float(f1_score(test_values, lr_pred, average="macro", zero_division=0)),
                "fit_status": "fit",
            }
        except ValueError:
            logistic[target_name] = {
                "accuracy": float(accuracy_score(test_values, majority_predictions)),
                "macro_f1": float(
                    f1_score(test_values, majority_predictions, average="macro", zero_division=0)
                ),
                "fit_status": "majority_class_fallback",
            }

        early_stopping = bool(config.baseline_mlp_early_stopping and class_counts.min() >= 2)
        ...
        with warnings.catch_warnings(record=True) as caught_warnings:
            ...
            try:
                if unique_classes.size < 2:
                    raise ValueError("single-class target")
                mlp_model.fit(x_train_full, train_values)
                ...
                mlp[target_name] = {
                    "accuracy": float(accuracy_score(test_values, mlp_pred)),
                    "macro_f1": float(
                        f1_score(test_values, mlp_pred, average="macro", zero_division=0)
                    ),
...
def ensure_trace_corpus(
    ...
    if rebuild:
        raise NotImplementedError(
            "E31 is self-contained but does not rebuild traces from raw data."
        )
    ...
    if missing:
        ...
        if fallback_missing:
            raise FileNotFoundError(
                f"Missing E31 trace cache files: {missing}. Fallback e29 cache is also incomplete: {fallback_missing}."
            )

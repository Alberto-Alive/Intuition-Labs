"""Smoke test for the DIGIT Extrapolation E9 package."""
...
assert hasattr(model.bottleneck, "prototype_bank"), "E9 should expose a diffusion-native prototype bank"
assert hasattr(model.bottleneck, "fragility_head"), "E9 should expose a fragility head"
...
assert torch.allclose(
    ...
    prim.certainty_score * prim.fragility_score,
    atol=1e-5,
), "effective_certainty must equal certainty_score * fragility_score"
...
assert torch.allclose(
    ...
    prim.effective_certainty * prim.decisiveness_score,
    atol=1e-5,
), "commitment_depth must equal effective_certainty * decisiveness_score"
assert torch.allclose(
...
assert torch.allclose(prim.success_proposal, prim.success_core, atol=1e-5), "success_proposal must track success_core"
assert not torch.allclose(
    prim.success_guard,
    prim.success_cert,
    atol=1e-5,
), "success_guard should not collapse to success_cert"

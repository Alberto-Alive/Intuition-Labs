...
def _anchor_geometry_from_model(model: DIGITModel) -> dict[str, Any]:
    del model
    return {"variant": "diffusion_prototype_e9"}
...
def _is_better_stage1(
    ...
    if abs(candidate_outcome - best_outcome) <= tolerance:
        ...
        if abs(candidate_failure - best_failure) <= tolerance:
            candidate_commitment_violation = _metric_or_default(
                metrics,
                "commitment_monotonicity_violation_rate",
                1e9,
            )
            best_commitment_violation = _metric_or_default(
                best_metrics,
                "commitment_monotonicity_violation_rate",
                1e9,
            )
            if candidate_commitment_violation < best_commitment_violation - tolerance:
                return True
            if abs(candidate_commitment_violation - best_commitment_violation) <= tolerance:
                candidate_success_excess = _success_rate_excess(metrics, config)
                best_success_excess = _success_rate_excess(best_metrics, config)
                if candidate_success_excess < best_success_excess - tolerance:
                    return True
                if abs(candidate_success_excess - best_success_excess) <= tolerance:
...
def main() -> None:
    ...
    print(
        ...
        "(retention: non-collapsed -> outcome_macro_f1 -> failure_recall -> lower commitment_violation -> lower success-rate excess -> trajectory_macro_f1 -> confidence_macro_f1 -> joint_acc; "
        "final selection: frontier(outcome/failure) -> lower commitment_violation -> lower success-rate excess -> trajectory_macro_f1 -> confidence_macro_f1 -> lower success_aurc -> joint_acc)"
    )

        ...
        if path_scales:
            ...
            if context_alias is not None:
                if "outcome_context" not in path_scales:
                    merged["outcome_context"] = float(context_alias)
                if "outcome_trajectory_context" not in path_scales:
                    merged["outcome_trajectory_context"] = float(context_alias)
                if "outcome_pattern_context" not in path_scales:
                    merged["outcome_pattern_context"] = float(context_alias)
            outcome_alias = path_scales.get("outcome_context")
            if outcome_alias is not None:
                if "outcome_trajectory_context" not in path_scales:
                    merged["outcome_trajectory_context"] = float(outcome_alias)
                if "outcome_pattern_context" not in path_scales:
                    merged["outcome_pattern_context"] = float(outcome_alias)

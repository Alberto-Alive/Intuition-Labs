"""Epistemic-anchor bottleneck scaffold for the Extrapolation E5 variant."""
...
class PrimitiveOutput(NamedTuple):
    ...
    worsening_radius: torch.Tensor
    worsening_ambiguity: torch.Tensor
    worsening_failure: torch.Tensor
...
class MonotoneSeverityHead(nn.Module):
    """Partial-monotone severity head that increases with ordered badness."""
    ...
    def forward(self, badness: torch.Tensor) -> torch.Tensor:
        hidden = F.relu(self.hidden(badness))
        severity = self.output(hidden) + self.bias
        scale = F.softplus(self.raw_scale).clamp_min(1e-4)
        return torch.sigmoid(scale * severity).squeeze(-1)
...
class EntropyBottleneckHead(nn.Module):
    ...
    def __init__(
        ...
        self.worsening_radius_head = MonotoneSeverityHead(
            input_dim=8,
            hidden_dim=monotone_hidden_dim,
            scale_init=2.0,
        )
        self.ambiguity_worsening_head = MonotoneSeverityHead(
            input_dim=8,
            hidden_dim=monotone_hidden_dim,
            scale_init=2.0,
        )
        self.failure_worsening_head = MonotoneSeverityHead(
            input_dim=8,
            hidden_dim=monotone_hidden_dim,
            scale_init=2.0,
        )
    ...
    def forward(
        ...
        outcome_attention_pattern_context = (
            attention_pattern_probs.detach() * scales["outcome_pattern_context"]
        )
        ...
        agreement = executor_feature_column(executor_features, "agreement").clamp(min=0.0, max=1.0)
        prob_margin = executor_feature_column(executor_features, "prob_margin").clamp(min=0.0, max=1.0)
        max_attention_mass = executor_feature_column(executor_features, "max_attention_mass").clamp(min=0.0, max=1.0)
        predictive_entropy = executor_feature_column(executor_features, "predictive_entropy").clamp(min=0.0, max=1.0)
        variation_ratio = executor_feature_column(executor_features, "variation_ratio").clamp(min=0.0, max=1.0)
        low_margin_flag = executor_feature_column(executor_features, "low_margin_flag").clamp(min=0.0, max=1.0)
        unstable_flag = executor_feature_column(executor_features, "unstable_flag").clamp(min=0.0, max=1.0)
        attention_top2_mass = executor_feature_column(executor_features, "attention_top2_mass").clamp(min=0.0, max=1.0)
        attention_effective_support = executor_feature_column(
            executor_features,
            "attention_effective_support",
        ).clamp(min=0.0, max=1.0)
        attention_gini = executor_feature_column(executor_features, "attention_gini").clamp(min=0.0, max=1.0)
        attention_concentration_drift = executor_feature_column(
            executor_features,
        ...
        trust_support_deficit = (1.0 - top_support).clamp(min=0.0, max=1.0)
        trust_approach_deficit = (1.0 - top_family_approach).clamp(min=0.0, max=1.0)
        trust_leap_cost = top_family_leap_penalty.clamp(min=0.0, max=1.0)
        trust_conflict = anchor_conflict.clamp(min=0.0, max=1.0)
        trust_agreement_deficit = (1.0 - agreement).clamp(min=0.0, max=1.0)
        trust_margin_deficit = torch.maximum(1.0 - prob_margin, low_margin_flag).clamp(min=0.0, max=1.0)
        concentration_strength = (
            0.28 * max_attention_mass
            + 0.24 * attention_top2_mass
            + 0.22 * attention_gini
            + 0.16 * (1.0 - attention_effective_support)
            + 0.10 * (1.0 - attention_concentration_drift)
        ).clamp(min=0.0, max=1.0)
        trust_concentration_deficit = (1.0 - concentration_strength).clamp(min=0.0, max=1.0)
        trust_variation_pressure = torch.maximum(variation_ratio, unstable_flag).clamp(min=0.0, max=1.0)
        trust_chart = torch.stack(
            [
        ...
        generic_worsening = trust_chart.mean(dim=-1)
        explicit_ambiguity = (
            0.24 * torch.maximum(boundary_support, anchor_conflict)
            + 0.16 * predictive_entropy
            + 0.14 * trust_agreement_deficit
            + 0.14 * trust_margin_deficit
            + 0.14 * trust_concentration_deficit
            + 0.08 * attention_concentration_drift
            + 0.10 * trust_variation_pressure
            + 0.07 * unstable_flag
            + 0.05 * low_margin_flag
            - 0.12 * failure_core
        ).clamp(min=0.0, max=1.0)
        explicit_failure = (
            0.38 * failure_core
            + 0.18 * failure_support
            + 0.12 * trust_agreement_deficit
            + 0.10 * trust_margin_deficit
            + 0.08 * trust_support_deficit
            + 0.06 * trust_variation_pressure
            + 0.04 * unstable_flag
            - 0.10 * boundary_support
            - 0.08 * anchor_conflict
        ).clamp(min=0.0, max=1.0)
        learned_ambiguity = self.ambiguity_worsening_head(trust_chart)
        learned_failure = self.failure_worsening_head(trust_chart)
        worsening_ambiguity = torch.lerp(explicit_ambiguity, learned_ambiguity, 0.35).clamp(min=0.0, max=1.0)
        worsening_failure = torch.lerp(explicit_failure, learned_failure, 0.35).clamp(min=0.0, max=1.0)
        explicit_worsening_radius = (
            0.50 * generic_worsening
            + 0.30 * worsening_ambiguity
            + 0.20 * worsening_failure
        ).clamp(min=0.0, max=1.0)
        learned_worsening_radius = self.worsening_radius_head(trust_chart)
        worsening_radius = torch.lerp(
            explicit_worsening_radius,
            learned_worsening_radius,
            0.35,
        ).clamp(min=0.0, max=1.0)

        base_explicit_commitment = self._explicit_commitment_depth(
        ...
        runner_up_pressure = (
            runner_up_family_support * (1.0 - family_margin)
        ).clamp(min=0.0, max=1.0)
        ambiguity_overlap = torch.maximum(
            boundary_core,
            torch.maximum(
                runner_up_pressure,
                anchor_conflict * (1.0 - 0.35 * family_margin),
            ),
        ).clamp(min=0.0, max=1.0)
        success_affinity = success_family_support
        success_base_quality = (
            0.35
            + 0.20 * success_family_approach
            + 0.15 * success_core
            + 0.15 * family_margin
            + 0.15 * base_explicit_commitment
        ).clamp(min=0.0, max=1.0)
        success_base_quality = (
            success_base_quality
            * (1.0 - 0.18 * runner_up_pressure)
            * (1.0 - 0.15 * ambiguity_overlap)
            * (1.0 - 0.10 * trust_concentration_deficit)
            * (1.0 - 0.55 * worsening_ambiguity)
            * (1.0 - 0.20 * worsening_radius)
            * (1.0 - 0.12 * worsening_failure)
        ).clamp(min=0.0, max=1.0)
        success_base = (success_affinity * success_base_quality).clamp(min=0.0, max=1.0)
        success_guard_pre = success_base
        ...
        success_proposal = success_guard_pre
        failure_decisiveness = (
            failure_structural
            * (
                0.45
                + 0.25 * failure_family_approach
                + 0.15 * family_margin
                + 0.10 * trust_agreement_deficit
                + 0.15 * worsening_failure
            )
            * (1.0 - 0.15 * ambiguity_overlap)
        ).clamp(min=0.0, max=1.0)
        failure_cert = torch.maximum(
            failure_decisiveness,
            (
                failure_support
                * (
                    0.20
                    + 0.35 * failure_family_approach
                    + 0.25 * worsening_failure
                    + 0.10 * trust_agreement_deficit
                    + 0.20 * family_margin
                )
            ).clamp(min=0.0, max=1.0),
        ).clamp(min=0.0, max=1.0)
        failure_agreement_floor = (
            failure_support
            * (
                0.12
                + 0.30 * failure_family_approach
                + 0.30 * trust_agreement_deficit
                + 0.14 * worsening_failure
                + 0.10 * trust_margin_deficit
            )
            * (1.0 - 0.10 * success_guard_pre)
        ).clamp(min=0.0, max=1.0)
        failure_cert = torch.maximum(failure_cert, failure_agreement_floor).clamp(min=0.0, max=1.0)
        ambiguity_attention_floor = (
            0.38 * trust_concentration_deficit
            + 0.20 * attention_concentration_drift
            + 0.16 * worsening_ambiguity
            + 0.12 * runner_up_pressure
            + 0.08 * anchor_conflict
            - 0.06 * failure_decisiveness
        ).clamp(min=0.0, max=1.0)
        ambiguity_cert = torch.maximum(
            ambiguity_overlap,
            (
                0.55 * worsening_ambiguity
                + 0.20 * worsening_radius
                + 0.15 * runner_up_pressure
                + 0.10 * anchor_conflict
                - 0.10 * failure_decisiveness
            ).clamp(min=0.0, max=1.0),
        ).clamp(min=0.0, max=1.0)
        ambiguity_cert = torch.maximum(ambiguity_cert, ambiguity_attention_floor).clamp(min=0.0, max=1.0)
        ambiguity_excess = torch.relu(ambiguity_cert - 0.35).clamp(min=0.0, max=1.0)
        guard_failure_relief = (1.0 - 0.55 * failure_cert).clamp(min=0.45, max=1.0)
        guard_runner_penalty = runner_up_pressure * (0.50 + 0.50 * ambiguity_excess)
        guard_radius_penalty = worsening_radius * (0.50 + 0.50 * ambiguity_excess)
        guard_concentration_penalty = torch.relu(
            torch.maximum(worsening_ambiguity, trust_concentration_deficit) - 0.20
        ).clamp(min=0.0, max=1.0)
        success_guard = (
            success_guard_pre
            * (
                1.0
                - 0.12 * guard_runner_penalty
                - 0.18 * ambiguity_excess * guard_failure_relief
                - 0.08 * guard_radius_penalty
                - 0.06 * guard_concentration_penalty
            ).clamp(min=0.30, max=1.0)
        ).clamp(min=0.0, max=1.0)
        success_proposal = success_guard
        success_cert = torch.minimum(
            success_guard,
            success_structural
            * (1.0 - 0.45 * failure_structural)
            * (1.0 - 0.30 * ambiguity_overlap)
            * (1.0 - 0.20 * worsening_radius),
        ).clamp(min=0.0, max=1.0)
        failure_cert = torch.maximum(
            failure_cert,
            (
                (failure_structural + 0.35 * worsening_failure * failure_support + 0.12 * trust_agreement_deficit)
                * (1.0 - 0.12 * success_guard)
                * (1.0 - 0.05 * ambiguity_cert)
            ).clamp(min=0.0, max=1.0),
        ).clamp(min=0.0, max=1.0)
        success_commitment = (
            success_guard
        ...
        explicit_commitment = (
            torch.maximum(success_commitment, failure_commitment)
            * (1.0 - 0.30 * ambiguity_cert)
            * (1.0 - 0.10 * worsening_radius)
            + 0.08 * family_margin
        ).clamp(min=0.0, max=1.0)
        ...
        outcome_logits = self._explicit_outcome_logits(
            ...
            worsening_radius=worsening_radius,
            worsening_ambiguity=worsening_ambiguity,
            worsening_failure=worsening_failure,
        )
    ...
    def _explicit_outcome_logits(
        ...
        *,
        ...
        failure_advantage = torch.relu(failure_cert - success_cert)
        success_advantage = torch.relu(success_cert - torch.maximum(ambiguity_cert, failure_cert))
        success_resolve = torch.relu(commitment_depth - ambiguity_cert)

        success_score = (
            success_cert
            + 0.15 * success_support
            + 0.20 * top_is_success
            + 0.35 * success_advantage
            + 0.12 * success_resolve
            - 0.75 * ambiguity_cert
            - 0.65 * failure_cert
            - 0.35 * worsening_ambiguity
            - 0.15 * worsening_radius
        )
        failure_score = (
            failure_cert
            ...
            - 0.10 * ambiguity_cert
            - 0.70 * success_cert
            + 0.20 * failure_advantage
            + 0.25 * worsening_failure
            - 0.05 * worsening_ambiguity
        )
        uncertain_score = (
            ambiguity_cert
            ...
            + 0.35 * worsening_ambiguity
            + 0.15 * worsening_radius
            - 0.10 * worsening_failure
            - 0.20 * failure_advantage
            - 0.30 * success_advantage
            - 0.12 * success_resolve
            - 0.08 * top_is_failure
        )

"""Loss functions for the DIGIT Extrapolation E5 scaffold variant."""
...
class DIGITLoss(nn.Module):
    """Combined loss for the anchor-structured E5 scaffold."""
        ...
        if perturbation_kind == "lower_attention_concentration":
            penalties.extend(
                [
                    ...
                    * (
                        ambiguity_context
                        * torch.relu(primitives.ambiguity_cert - perturbed_primitives.ambiguity_cert)
                    ).mean(),
                    1.25
                    * (
                        ambiguity_context
                        * torch.relu(base_uncertain_prob - perturbed_uncertain_prob)
                    ).mean(),
                    1.25
                    * (
                        ambiguity_context
            ...
            if hasattr(primitives, "worsening_ambiguity"):
                penalties.append(
                    ...
                    * (
                        ambiguity_context
                        * torch.relu(primitives.worsening_ambiguity - perturbed_primitives.worsening_ambiguity)
                    ).mean()
                )
        ...
        if perturbation_kind == "lower_agreement":
            penalties.extend(
                [
                    1.5
                    * (
                        agreement_failure_context
                        * torch.relu(primitives.failure_cert - perturbed_primitives.failure_cert)
                    ).mean(),
                    1.25
                    * (
                        agreement_failure_context
                        * torch.relu(base_failure_prob - perturbed_failure_prob)
                    ).mean(),
                    1.10
                    * (
                        agreement_failure_context
                        * torch.relu(
                            primitives.failure_family_support - perturbed_primitives.failure_family_support
                        )
                    ).mean(),
                ]
            )
            if hasattr(primitives, "worsening_failure"):
                penalties.append(
                    ...
                    * (
                        agreement_failure_context
                        * torch.relu(primitives.worsening_failure - perturbed_primitives.worsening_failure)
                    ).mean()
                )
        if perturbation_kind == "higher_entropy":
            penalties.extend(
                [
                    ...
                    * (
                        failure_context
                        * torch.relu(perturbed_guard_gap - base_guard_gap)
                    ).mean(),
                ]
            )
            if hasattr(primitives, "worsening_failure"):

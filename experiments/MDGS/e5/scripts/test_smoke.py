...
print(f"Anchor state: {tuple(out['primitives'].anchor_state.shape)}")
print(f"Anchor assignment probs: {tuple(out['primitives'].anchor_assignment_probs.shape)}")
...
assert out["primitives"].anchor_state.shape[-1] == config.anchor_state_dim
assert out["primitives"].anchor_assignment_probs.shape[1] == 3
assert out["primitives"].anchor_assignment_probs.shape[2] == config.anchors_per_family
assert torch.isfinite(out["primitives"].commitment_depth).all()
assert torch.isfinite(out["primitives"].anchor_conflict).all()
assert torch.isfinite(out["primitives"].anchor_support_scores).all()
...
if (trace_dir / "train.jsonl").exists():
    ...
    print(f"Trace input dim: {trace_inputs.shape[0]}")
    print(f"Trace executor features: {tuple(trace_out['executor_features'].shape)}")

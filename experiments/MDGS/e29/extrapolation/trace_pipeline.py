"""Local trace-cache helpers and simple baselines for E29."""
...
def ensure_trace_corpus(
    ...
    if rebuild:
        raise NotImplementedError(
            "E29 is self-contained but does not rebuild traces from raw data. Use the local e29 trace_cache files."
        )
    if missing:
        raise FileNotFoundError(
            f"Missing E29 trace cache files: {missing}. Expected local files under {root_dir}."
        )

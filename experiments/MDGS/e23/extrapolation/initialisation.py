"""Shared initialisation helpers for DIGIT Extrapolation E23."""
...
@dataclass
class E23InitialisedModels:
    """The three E23 models after applying the frozen shared initialisation rule."""

    baseline: BaselineMLP
    e23: DualParameterAttentionMLP
...
def _make_e23_model(config: E23Config) -> DualParameterAttentionMLP:
...
def _make_ablation_model(config: E23Config) -> ActiveLearnedMemoryMLP:
...
def _make_baseline_model(config: E23Config) -> BaselineMLP:
    return BaselineMLP(
        part_a_vocab_size=config.num_part_a_values,
        part_b_vocab_size=config.num_part_b_values,
        embedding_dim=config.embedding_dim,
...
def build_initial_slot_tensors(config: E23Config) -> tuple[torch.Tensor, torch.Tensor]:

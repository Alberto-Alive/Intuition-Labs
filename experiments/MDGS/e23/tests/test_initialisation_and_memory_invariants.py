"""Pre-training invariant tests for the DIGIT Extrapolation E23 models."""
...
from experiments.DIGIT.Extrapolation.e23.extrapolation.config import E23Config
from experiments.DIGIT.Extrapolation.e23.extrapolation.initialisation import initialise_e23_models


SEEDS = (0, 1, 2, 3, 4)


def _make_config(seed: int = 0) -> E23Config:
    return E23Config(seed=seed)
...
def test_models_2_and_3_have_identical_non_memory_trainable_tensors_at_step_0_for_every_seed() -> None:
    for seed in SEEDS:
        initialized = initialise_e23_models(_make_config(seed))
        e23_named_parameters = dict(initialized.e23.named_parameters())
        ...
        assert set(e23_named_parameters) == set(ablation_named_parameters)
        for name, parameter in e23_named_parameters.items():
...
def test_models_2_and_3_have_identical_effective_memory_tensors_at_step_0_for_every_seed() -> None:
    for seed in SEEDS:
        ...
        assert torch.equal(initialized.e23.layer1.prototypes, initialized.ablation.layer1.raw_slots)
        assert torch.equal(initialized.e23.layer2.prototypes, initialized.ablation.layer2.raw_slots)
        assert torch.allclose(
            initialized.e23.layer1.effective_memory(),
            initialized.ablation.layer1.effective_memory().detach(),
        )
        assert torch.allclose(
            initialized.e23.layer2.effective_memory(),
...
def test_model_2_prototypes_are_buffers_and_excluded_from_the_optimizer() -> None:
    initialized = initialise_e23_models(_make_config(0))
...
def test_there_is_no_empty_slot_warmup_and_both_models_start_fully_valid_with_unit_gate() -> None:
    for seed in SEEDS:
        ...
        e23_outputs = initialized.e23(part_a, part_b)
        ablation_outputs = initialized.ablation(part_a, part_b)

        assert bool(initialized.e23.layer1.valid_mask.all())
        assert bool(initialized.e23.layer2.valid_mask.all())
        assert bool(initialized.ablation.layer1.valid_mask.all())
        assert bool(initialized.ablation.layer2.valid_mask.all())
        assert initialized.e23.layer1.occupancy == 32
        assert initialized.e23.layer2.occupancy == 32
        assert initialized.ablation.layer1.occupancy == 32
        assert initialized.ablation.layer2.occupancy == 32
        assert torch.allclose(e23_outputs.layer1_occupancy_gate, torch.tensor(1.0))
        assert torch.allclose(e23_outputs.layer2_occupancy_gate, torch.tensor(1.0))

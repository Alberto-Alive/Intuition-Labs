"""Configuration dataclass for the DIGIT Extrapolation E10 experiment."""
...
class Config:
    ...
    lambda_pattern_margin: float = 0.15
    ...
    trace_stage1_rescue_epochs: int = 6
    trace_stage1_rescue_force_barrier: bool = True
    ...
    trace_stage1_frontier_pattern_slack: float = 0.05
    trace_stage1_frontier_diffuse_slack: float = 0.05
    ...
    anchor_repulsion_weight: float = 0.25
    anchor_usage_weight: float = 0.20
    anchor_approach_alignment_weight: float = 0.12
    anchor_family_target_weight: float = 0.30
    anchor_family_margin_weight: float = 0.25
    anchor_family_margin: float = 0.10
    anchor_family_activity_weight: float = 0.35
    anchor_family_activity_ratio: float = 0.75
    anchor_success_family_weight: float = 1.10
    anchor_failure_family_weight: float = 1.75
    anchor_failure_boundary_margin_weight: float = 0.20
    anchor_failure_boundary_margin: float = 0.05
    ...
    anchor_repulsion_margin: float = 1.00
    monotone_confidence_threshold_init: float = 0.30
    monotone_confidence_threshold_gap_init: float = 0.25
    ...
    trace_outcome_failure_weight: float = 6.5
    ...
    trace_success_prior_weight: float = 0.15
    trace_success_prior_slack: float = 0.08
    trace_guard_quality_weight: float = 0.12
    trace_selection_success_rate_max: float = 0.34
    trace_barrier_warmup_epochs: int = 2
    ...
    trace_force_stage1_only: bool = True
    trace_require_noncollapsed_stage1_candidate: bool = True
    ...
    trace_path_scale_outcome_context: float = 1.0
    trace_path_scale_outcome_trajectory_context: float = 1.0
    trace_path_scale_outcome_pattern_context: float = 1.0
    ...
    trace_pattern_prior_weight: float = 1.50
    trace_pattern_logit_margin: float = 0.15
    trace_outcome_evidence_scale: float = 1.0
    trace_outcome_trajectory_scale: float = 1.0
    trace_outcome_pattern_scale: float = 0.80
    baseline_mlp_max_iter: int = 2000
    baseline_mlp_early_stopping: bool = True

    # ── Cooperative diffusion evidence + prototype geometry (E10) ────────────
    ...
    # Hidden dim kept for compatibility; E10 derives fragility from cooperative support geometry.
    diffusion_fragility_hidden: int = 96
    # Softmax temperature used when assigning reconstructed path states to prototypes.
    prototype_support_temperature: float = 0.18
    ...
    # Auxiliary E10 losses.
    lambda_fragility_target: float = 0.35
    lambda_perturbation_rank: float = 0.10
    lambda_perturbation_proximity: float = 0.05
    lambda_basin_order: float = 0.15
    lambda_ordered_geometry: float = 0.15
    # Margin for the paired base-vs-perturbed denoise ranking loss.
    ...
    trace_outcome_success_weight: float = 1.00
    trace_outcome_uncertain_weight: float = 1.25
    lambda_path_evidence_targets: float = 0.60
    lambda_cooperative_head_targets: float = 0.45

...
class DiffusionBottleneckHead(nn.Module):
    ...
    def __init__(
        ...
        **_kwargs,
        ...
        del fragility_hidden, confidence_threshold_init, confidence_threshold_gap_init
        ...
        self.confidence_head = _make_mlp(12, max(32, hidden_dim // 3), num_confidence, dropout)
        self.outcome_head = _make_mlp(
            12 + num_trajectory + num_pattern,
            max(64, hidden_dim // 2),
            num_outcome,
            dropout,
        )
    ...
    def _run_random_paths(
        ...
        *,
        ...
        z_rec_list, mse_list = [], []
        effective_seed = path_seed
        if effective_seed is None and not self.training:
            effective_seed = 0
        rng = None
        if effective_seed is not None:
            rng = torch.Generator(device="cpu")
            rng.manual_seed(int(effective_seed))
        ...
        for _ in range(self.num_random_paths):
            if rng is None:
                ...
                t = torch.randint(1, self.T + 1, (bsz,), generator=rng, device="cpu").to(device=device)
                eps = torch.randn(z0.shape, generator=rng, device="cpu", dtype=torch.float32).to(
                    device=device,
                    dtype=z0.dtype,
                )
            z_t, eps_true = self._noise_z(z0, t, eps=eps)
        ...
        return z_rec_stack, denoise_mse
    ...
    def forward(
        ...
        if path_scales:
            if "shared_trace" in path_scales:
                ...
                if "success_family" not in path_scales:
                    scales["success_family"] = trace_scale
                if "failure_family" not in path_scales:
                    scales["failure_family"] = trace_scale
                if "boundary_family" not in path_scales:
                    scales["boundary_family"] = trace_scale
        ...
        z_rec_stack, denoise_mse = self._run_random_paths(z0, path_seed=path_seed)
        ...
        family_scale = z_q.new_tensor(
            [
                scales["success_family"],
                scales["failure_family"],
                scales["boundary_family"],
            ]
        ).view(1, 1, -1)
        prototype_family_support_paths = (prototype["family_support_paths"] * family_scale).clamp(min=0.0, max=1.0)
        prototype_family_distribution_paths = prototype_family_support_paths.clamp_min(1e-6)
        prototype_family_distribution_paths = prototype_family_distribution_paths / prototype_family_distribution_paths.sum(
            dim=-1,
            keepdim=True,
        ).clamp_min(1e-6)
        family_support = prototype_family_support_paths.mean(dim=0)
        family_distribution = family_support.clamp_min(1e-6)
        family_distribution = family_distribution / family_distribution.sum(dim=-1, keepdim=True).clamp_min(1e-6)
        prototype_order_score_paths = _ordered_score_from_family_distribution(prototype_family_distribution_paths)
        prototype_order_score = prototype_order_score_paths.mean(dim=0)
        prototype_order_score_spread = prototype_order_score_paths.std(dim=0, unbiased=False)
        topk = torch.topk(family_support, k=min(2, family_support.size(-1)), dim=-1).values
        prototype_top_support = topk[:, 0].clamp(min=0.0, max=1.0)
        runner_up = topk[:, 1] if topk.size(-1) > 1 else torch.zeros_like(prototype_top_support)
        prototype_family_margin = (
            (prototype_top_support - runner_up).clamp_min(0.0)
            / (prototype_top_support + runner_up).clamp_min(1e-6)
        ).clamp(min=0.0, max=1.0)
        family_entropy = -(
            family_distribution * family_distribution.log()
        ).sum(dim=-1) / math.log(family_support.size(-1))
        non_top_support = (
            (family_support.sum(dim=-1) - prototype_top_support) / max(family_support.size(-1) - 1, 1)
        ).clamp(min=0.0, max=1.0)
        prototype_overlap = (non_top_support / prototype_top_support.clamp_min(1e-6)).clamp(min=0.0, max=1.0)
        anchor_conflict = (0.55 * family_entropy + 0.45 * prototype_overlap).clamp(min=0.0, max=1.0)
        prototype_total_support = (
            family_support.sum(dim=-1) / max(family_support.size(-1), 1)
        ).clamp(min=0.0, max=1.0)
        family_approach = (prototype["family_approach"] * scales["approach"]).clamp(min=0.0, max=1.0)
        family_leap_penalty = (prototype["family_leap_penalty"] * scales["leap"]).clamp(min=0.0, max=1.0)
        family_weights = family_support.clamp_min(1e-6)
        family_weights = family_weights / family_weights.sum(dim=-1, keepdim=True).clamp_min(1e-6)
        approach_signal = (family_weights * family_approach).sum(dim=-1).clamp(min=0.0, max=1.0)
        leap_signal = (family_weights * family_leap_penalty).sum(dim=-1).clamp(min=0.0, max=1.0)
        ...
        support_badness = torch.stack(
            [
                ...
                (1.0 - prototype_top_support).clamp(0.0, 1.0),
                (1.0 - prototype_total_support).clamp(0.0, 1.0),
                ...
                torch.maximum(
                    ...
                    0.5 * (1.0 - approach_signal + leap_signal),
                ),
            ],
            dim=-1,
        )
        ...
        support_native = torch.clamp(
            0.21 * aggregated["support_mean"]
            + 0.12 * agreement
            + 0.09 * prob_margin
            + 0.08 * max_att_mass
            + 0.10 * attention_effective_support
            + 0.10 * prototype_total_support
            + 0.08 * prototype_family_margin
            + 0.07 * (1.0 - prototype_overlap)
            + 0.05 * approach_signal
            + 0.05 * (1.0 - leap_signal)
            + 0.05 * (1.0 - prototype["nearest_anchor_distance"]),
            min=0.0,
            max=1.0,
        )
        ...
        success_family_approach = family_approach[:, 0]
        failure_family_approach = family_approach[:, 1]
        boundary_family_approach = family_approach[:, 2]
        success_family_leap_penalty = family_leap_penalty[:, 0]
        failure_family_leap_penalty = family_leap_penalty[:, 1]
        boundary_family_leap_penalty = family_leap_penalty[:, 2]
        success_affinity = torch.relu(
            success_family_support - torch.maximum(boundary_family_support, failure_family_support)
        ).clamp(0.0, 1.0)
        failure_pressure = torch.relu(
            failure_family_support - torch.maximum(boundary_family_support, success_family_support)
        ).clamp(0.0, 1.0)
        boundary_uncertainty = torch.relu(
            boundary_family_support - failure_family_support
        ).clamp(0.0, 1.0)
        ...
        trust_approach_deficit = (
            0.55 * aggregated["path_disagreement"] + 0.45 * (1.0 - approach_signal)
        ).clamp(0.0, 1.0)
        trust_leap_cost = (
            0.70 * aggregated["energy_mean_norm"] + 0.30 * leap_signal
        ).clamp(0.0, 1.0)
        trust_conflict = (
            0.25 * aggregated["outcome_entropy_norm"]
            + 0.15 * aggregated["path_disagreement"]
            + 0.15 * anchor_conflict
            + 0.15 * prototype_overlap
            + 0.10 * aggregated["boundary_mean"]
            + 0.10 * trust_approach_deficit
            + 0.10 * trust_leap_cost
        ).clamp(0.0, 1.0)
        ...
        success_guard = torch.lerp(
            torch.ones_like(success_guard),
            success_guard,
            success_guard.new_full(success_guard.shape, scales["barrier"]),
        ).clamp(0.0, 1.0)
        ...
        success_evidence = (
            0.84 * success_base
            + 0.11 * success_family_support
            + 0.05 * success_affinity
        ).clamp(0.0, 1.0)
        failure_evidence = (
            0.82 * failure_base
            + 0.12 * failure_family_support
            + 0.06 * failure_pressure
        ).clamp(0.0, 1.0)
        ambiguity_cert = (
            0.68 * ambiguity_base
            + 0.12 * aggregated["boundary_mean"]
            + 0.05 * boundary_uncertainty
            + 0.10 * (1.0 - support_cert)
            + 0.03 * anchor_conflict
        ).clamp(0.0, 1.0)
        ...
        confidence_context = torch.cat(
            [
                ...
                * torch.stack(
                ...
                0.20 * prototype_family_margin.unsqueeze(-1),
            ],
            dim=-1,
        )
        confidence_logits = self.confidence_head(confidence_context)
        family_context = torch.stack(
            [
                success_family_support,
                failure_family_support,
                boundary_family_support,
        ...
        outcome_context = torch.cat(
            [
                self.outcome_evidence_scale * scales["outcome_context"] * evidence_vector,
                self.outcome_evidence_scale * scales["outcome_context"] * cert_context,
                0.20 * self.outcome_evidence_scale * scales["outcome_context"] * family_context,
                self.outcome_trajectory_scale * scales["outcome_trajectory_context"] * traj_probs.detach(),
                self.outcome_pattern_scale * scales["outcome_pattern_context"] * pattern_probs.detach(),
            ],
            dim=-1,
        )
        ...
        return PrimitiveOutput(
            ...
            prototype_family_support_paths=prototype_family_support_paths,
            prototype_family_distribution_paths=prototype_family_distribution_paths,

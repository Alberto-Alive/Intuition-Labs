...
def _is_better_stage1(
    ...
    candidate_key = _stage1_selection_key(metrics, config)
    best_key = _stage1_selection_key(best_metrics, config)
    if candidate_key[0] > best_key[0] + tolerance:
        return True
    if candidate_key[0] < best_key[0] - tolerance:
        return False
...
def _stage1_candidate_retention_key(candidate: dict[str, Any]) -> tuple[float, ...]:
    ...
    return (float(int(not collapsed)), *_stage1_selection_key(metrics, config))
...
def _select_stage1_candidate(
    ...
    if non_collapsed:
        # The candidate bank already preserves the strongest late checkpoints.
        # Avoid hard frontier cliffs that can discard clearly better all-around epochs.
        return max(pool, key=lambda candidate: _stage1_selection_key(candidate["metrics"], config))

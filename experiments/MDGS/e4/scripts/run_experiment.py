...
def _anchor_geometry_from_model(model: DIGITModel) -> dict[str, Any]:
    ...
    centers = bottleneck.anchor_centers_param.detach().cpu()
    scales = F.softplus(bottleneck.anchor_raw_scales.detach().cpu()).clamp_min(1e-4)
    directions = F.normalize(bottleneck.anchor_directions_param.detach().cpu(), dim=-1)
    leap_scales = F.softplus(bottleneck.anchor_raw_leap_scales.detach().cpu()).clamp_min(1e-4)
    payload = {}
    for family_idx, family_name in enumerate(["success", "failure", "boundary"]):
        payload[family_name] = {
            "center_norm": _tensor_summary_with_quantiles(centers[family_idx].norm(dim=-1)),
            "radius_mean": _tensor_summary_with_quantiles(scales[family_idx].mean(dim=-1)),
            "radius_max": _tensor_summary_with_quantiles(scales[family_idx].max(dim=-1).values),
            "direction_norm": _tensor_summary_with_quantiles(directions[family_idx].norm(dim=-1)),
            "leap_scale": _tensor_summary_with_quantiles(leap_scales[family_idx]),
        }
    return payload
...
def _commitment_monotonicity_audit(
    ...
    per_kind: dict[str, dict[str, float]] = {
        kind: {"count": 0.0, "commitment": 0.0, "success_prob": 0.0}
...
def main() -> None:
    ...
    print(f"DIGIT Extrapolation {variant_name.upper()} epistemic-anchor run")
    ...
    for epoch in range(config.trace_stage1_epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            queries, trace_inputs, evidence_targets, prim_targets, target_ids = _move_batch(batch, device)
            amp_context = autocast(enabled=use_amp) if use_amp else nullcontext()
            with amp_context:
                out = model(
                    queries,
                    trace_inputs,

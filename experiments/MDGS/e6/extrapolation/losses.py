"""Loss functions for the DIGIT Extrapolation E6 diffusion variant.

Changes from E5:
  - Adds lambda_diffusion_denoise * diffusion_denoise_loss (denoising MSE).
  - Drops all anchor-specific losses (anchor_pull, anchor_repulsion,
    anchor_usage, anchor_approach_alignment, anchor_family_target,
    anchor_family_margin, cooperative_cert_order, cooperative_guard_target,
    cooperative_commitment_target) because the E6 bottleneck only exposes
    compatibility stubs for those fields.
  - Outcome supervision now applies directly to the diffusion-owned
    outcome_logits; there is no separate clean-latent bootstrap path.
"""
...
class DIGITLoss(nn.Module):
    """Combined loss for the E6 diffusion-structured scaffold."""
    ...
    def forward(
        ...
        # ── Diffusion denoising MSE (E6-specific) ─────────────────────────────

"""Model definitions for DIGIT Extrapolation E24."""
...
class MemoryLayerForwardResult:
    """Forward outputs exposed by one E24 hidden layer."""
...
class DualParameterForwardResult:
    """Forward outputs needed for E24 invariant tests."""
    ...
    layer1_attention_gate: torch.Tensor
    layer2_attention_gate: torch.Tensor
...
class ExperienceMemoryAttentionLayer(nn.Module):
    """E24 familiarity layer with detached experience-updated prototype buffers."""
    ...
    def forward(self, inputs: torch.Tensor) -> MemoryLayerForwardResult:
        """Run semantic projection and memory attention before gate application."""
...
class LearnedMemoryAttentionLayer(nn.Module):
    """E24 learned-memory ablation layer with gradient-trained slot parameters."""
...
class DualParameterAttentionMLP(nn.Module, _InputEncodingMixin):
    """Model 2 from E24 with gated experience-updated prototype memory."""
    ...
    def __init__(
        ...
        *,
        ...
        self.g_1 = nn.Parameter(torch.zeros(()))
        self.g_2 = nn.Parameter(torch.zeros(()))
    ...
    def forward(
        ...
        """Run the approved E24 gated dual-parameter forward computation."""
        ...
        layer1_attention_gate = torch.sigmoid(self.g_1).to(
            dtype=layer1.h_semantic.dtype,
            device=layer1.h_semantic.device,
        )
        layer1_output = layer1.h_semantic + layer1_attention_gate * layer1.attention_output

        layer2 = self.layer2(layer1_output)
        layer2_attention_gate = torch.sigmoid(self.g_2).to(
            dtype=layer2.h_semantic.dtype,
            device=layer2.h_semantic.device,
        )
        layer2_output = layer2.h_semantic + layer2_attention_gate * layer2.attention_output

        logits = self.classifier(layer2_output)
        return DualParameterForwardResult(
            ...
            layer1_attention_gate=layer1_attention_gate,
            layer2_attention_gate=layer2_attention_gate,
        )
    ...
    @torch.no_grad()
    def set_gates(self, gate_init: float) -> None:
        """Set both learned scalar gates to the frozen initial value."""
        self.g_1.fill_(gate_init)
        self.g_2.fill_(gate_init)
...
class ActiveLearnedMemoryMLP(nn.Module, _InputEncodingMixin):
    """Model 3 from E24 with gated gradient-trained learned memory slots."""
    ...
    def forward(
        ...
        """Run the approved E24 gated learned-memory ablation forward computation."""
        input_embedding = self.encode_inputs(
            part_a,
            part_b,
            part_a_noise=part_a_noise,
...
class BaselineMLP(nn.Module, _InputEncodingMixin):
    """Model 1 from E24: the matched semantic-only baseline."""

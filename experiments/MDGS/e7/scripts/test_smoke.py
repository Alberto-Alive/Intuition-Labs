"""Smoke test for the DIGIT Extrapolation E7 package."""
...
assert hasattr(model.bottleneck, "prototype_bank"), "E7 should expose a diffusion-native prototype bank"
assert hasattr(model.bottleneck, "fragility_head"), "E7 should expose a fragility head"
...
assert hasattr(prim, "prototype_alignment_loss"), "prototype_alignment_loss field missing"
assert torch.isfinite(prim.prototype_alignment_loss), "prototype_alignment_loss is non-finite"

# E7-specific auxiliary losses
...
print(f"Prototype alignment loss: {prim.prototype_alignment_loss.item():.4f}")
...
assert torch.allclose(
    ...
    prim.certainty_score * prim.fragility_score * prim.decisiveness_score,
    atol=1e-5,
), "commitment_depth must equal certainty_score * fragility_score * decisiveness_score"
print("Diffusion/prototype invariants: OK")

...
class NextTokenWindowDataset(Dataset):
    def __init__(self, texts: Sequence[str], vocab: Vocabulary, sequence_length: int) -> None:
        ...
        for text in texts:
            ...
            for start in range(0, len(token_ids) - sequence_length):
                ...
                rows.append(
                    {
                        "input_ids": torch.as_tensor(token_ids[start:stop], dtype=torch.long),
                        "target_id": torch.as_tensor(token_ids[stop], dtype=torch.long),
...
class SparseLocalCloneMetaAttentionLM(nn.Module):
    ...
    def forward(
        ...
        return_meta_attention: bool = False,
    ) -> Dict[str, object]:
        ...
        meta_state, meta_weights = self.meta_attention(pooled)
        joint_logits = self.lm_head(meta_state)
        ...
        if return_meta_attention:
            out["meta_state"] = meta_state
        return out
...
def _build_corpus_splits(config: CorpusConfig, seed: int) -> tuple[Dict[str, List[str]], Dict[str, object]]:
    dataset_config = MultiViewCodePatchDatasetConfig(
        ...
        generator_version="real_import_restore_candidate_balanced_34b_stage14_sparse_local_clone_meta_attention",
...
def _joint_loss(joint_logits: torch.Tensor, target_id: torch.Tensor) -> torch.Tensor:
    return F.cross_entropy(joint_logits, target_id)
...
def evaluate_model(
    model: SparseLocalCloneMetaAttentionLM,
    loader: DataLoader,
    device: torch.device,
) -> Dict[str, object]:
    model.eval()
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            joint_loss = F.cross_entropy(joint_logits, target_id, reduction="sum")
            joint_nll += float(joint_loss.detach().cpu().item())
            token_count += int(target_id.shape[0])

            for clone_index in range(model.config.num_clones):
                clone_logits = model.lm_head(clone_states[:, clone_index])
                clone_loss = F.cross_entropy(clone_logits, target_id, reduction="sum")
                clone_nll[clone_index] += clone_loss.detach()
                clone_tokens[clone_index] += float(target_id.shape[0])
    ...
    merged_clone_mask = (
        torch.cat(clone_mask_batches, dim=0)
        if clone_mask_batches
        else torch.zeros(1, model.config.num_clones, model.max_length, dtype=torch.bool)
    )
...
def train_on_text_splits(
    ...
    train_dataset = NextTokenWindowDataset(text_splits["train"], vocab=vocab, sequence_length=int(config.corpus.max_length))
    dev_dataset = NextTokenWindowDataset(text_splits["dev"], vocab=vocab, sequence_length=int(config.corpus.max_length))
    test_dataset = NextTokenWindowDataset(text_splits["test"], vocab=vocab, sequence_length=int(config.corpus.max_length))
        ...
        for batch in train_loader:
            ...
            with _amp_context(device, bool(config.training.use_bf16)):
                outputs = model(
                    ...
                    return_meta_attention=False,
                )
                loss = _joint_loss(outputs["joint_logits"], target_id)
            loss.backward()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Stage 14 sparse-local clone meta-attention.")

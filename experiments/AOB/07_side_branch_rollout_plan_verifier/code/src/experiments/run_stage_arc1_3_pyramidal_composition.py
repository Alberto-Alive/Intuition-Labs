...
def _render_report(result: Dict[str, object]) -> str:
    ...
    lines.extend(
        [
            ...
            "3. Does token-level composition beat pooled composition? Compare `P1_pairwise_pooled_*` against `P2_pairwise_token_*` rows in the leaderboard; this smoke run does not promote a Phase 2 claim unless gates pass.",
            "4. Does candidate-guided composition help or leak? `P6_candidate_guided_composition` runs the same candidate-only, metadata, mismatch, shuffle, and structural leakage audits; candidate index/source embeddings are not used.",
            "5. Does root-level composition add value beyond pairwise composition? Compare `P3_binary_tree_root_intermediate`, `P5_recursive_weight_tied_composer`, and root ablations.",
...
def _variant_plan(config_variants: object = None) -> List[PyramidalVariant]:
    ...
    if config_variants:
        ...
        variants = [variant for variant in variants if variant.name in allowed or variant.stage_variant in allowed]
    return variants

...
SPEC = importlib.util.spec_from_file_location("stage16_exploratory_combination_search", MODULE_PATH)
...
def test_relative_support_routing_assigns_role_windows() -> None:
    model = MODULE.QuestionRoutedMultiHopQACloneModel(
        vocab_size=32,
        question_max_length=4,
        window_size=2,
        config=MODULE.ModelConfig(num_clones=4, hidden_size=8, num_layers=1, num_heads=2, ff_dim=16, dropout=0.0, routing_mode="relative_support"),
    )
    weights, selected = model._relative_route_support(
        support_windows=[[1, 3]],
        answer_window=torch.tensor([2], dtype=torch.long),
        device=torch.device("cpu"),
    )
    assert weights.shape == (1, 4, 4)
    assert selected.tolist()[0] == [1, 3, 2, 0]


def test_relative_support_routing_falls_back_to_answer_anchor() -> None:
    model = MODULE.QuestionRoutedMultiHopQACloneModel(
        vocab_size=32,
        question_max_length=4,
        window_size=2,
    ...
    _weights, selected = model._relative_route_support(
        support_windows=[[]],
        answer_window=torch.tensor([2], dtype=torch.long),
        device=torch.device("cpu"),
    )
    assert selected.tolist()[0] == [2, 1, 3, 0]


def test_learned_relative_support_targets_match_relative_roles() -> None:
    targets = MODULE._support_targets(
        support_windows=[[3, 1]],
        answer_window=torch.tensor([2], dtype=torch.long),
        num_clones=4,
        num_windows=4,
    ...
    # min support, max support, answer, then proximity filler -> [1, 3, 2, 0]
    assert targets.tolist()[0] == [1, 3, 2, 0]


def test_relative_role_order_is_shift_invariant() -> None:
    base = MODULE._relative_role_order([1, 3], 2, 8)
    shifted = MODULE._relative_role_order([2, 4], 3, 8)
    # A uniform +1 position shift maps role-by-role: relative roles do not depend
    # on absolute window indices, only on the support/answer relationships.
    assert [value + 1 for value in base if value + 1 < 8][:3] == shifted[:3]


def test_learned_relative_routing_ignores_gold_labels_at_eval() -> None:
    model = MODULE.QuestionRoutedMultiHopQACloneModel(
        vocab_size=64,
        question_max_length=4,
        window_size=2,
    ...
    model.eval()
    input_ids = torch.randint(0, 63, (2, model.total_length), dtype=torch.long)
    attention_mask = torch.ones(2, model.total_length, dtype=torch.bool)
    context_mask = torch.ones(2, model.context_length, dtype=torch.bool)

    def selected(support_windows, answer_window):
        with torch.no_grad():
            out = model(
                input_ids=input_ids,
        ...
        return out["selected_windows"]

    first = selected([[0, 1], [2, 3]], torch.tensor([0, 1], dtype=torch.long))
    # Completely different (even adversarial) gold labels must not move routing,
    # because the learned_relative router only reads question/context features.
    second = selected([[3, 2], [0, 3]], torch.tensor([3, 2], dtype=torch.long))
    assert torch.equal(first, second)
    # And distinct windows per clone, like learned_distinct.
...
def test_discrete_message_mode_emits_bounded_tokens() -> None:
    model = MODULE.QuestionRoutedMultiHopQACloneModel(
        vocab_size=64,
        question_max_length=4,
        window_size=2,
    ...
    assert "message_tokens" in outputs
    assert outputs["message_tokens"].shape == (1, 4)
    assert int(outputs["message_tokens"].max().item()) < 5
...
def test_train_on_prepared_splits_smoke_runs_and_reports_routing_metrics() -> None:
    ...
    config = MODULE.Stage15cConfig(
        ...
        curriculum=MODULE.CurriculumConfig(
            mask_pretrain_epochs=1,
            support_warmup_epochs=1,
            qa_finetune_epochs=2,
            use_mask_pretrain=True,
            use_support_warmup=True,
        ),
    )
    results = MODULE.train_curriculum_on_prepared_splits(splits, config)

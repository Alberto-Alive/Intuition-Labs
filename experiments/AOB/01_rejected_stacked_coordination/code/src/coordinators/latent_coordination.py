...
class CandidateTokenCrossAttentionCoordinator(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        model_dim = _compatible_dim(config.model_dim, config.num_heads)
        self.num_blocks = max(1, int(config.num_layers))
        self.shared_block_weights = bool(config.shared_block_weights)
        self.block_dropout = float(config.block_dropout)
        self.capture_diagnostics = False
        self.disabled_blocks: set[int] = set()
        self.zero_candidate_features = False
        self.zero_token_states = False
        self.last_diagnostics: dict[str, object] = {}
        self.token_projection = nn.Linear(config.input_dim, model_dim)
        self.candidate_projection = nn.Linear(candidate_feature_dim, model_dim)
        self.candidate_dropout = nn.Dropout(float(config.candidate_dropout))
        self.role_embedding = nn.Embedding(n_roles, model_dim)
        layer_count = 1 if self.shared_block_weights else self.num_blocks
        self.layers = nn.ModuleList(
            [
                _CrossAttentionBlock(
                    ...
                    config.ff_dim,
                    config.dropout,
                    use_residual_update=bool(config.use_residual_update),
                    use_layer_norm=bool(config.use_layer_norm),
                    use_feedforward=bool(config.use_feedforward),
                )
    ...
    def forward(
        ...
        if bool(self.zero_token_states):
            token_states = torch.zeros_like(token_states)
        if bool(self.zero_candidate_features):
        ...
        queries = self.candidate_dropout(self.candidate_projection(candidate_features))
        capture = bool(getattr(self, "capture_diagnostics", False))
        early_logits = []
        attention_weights = []
        representations = [queries.detach().float().cpu()] if capture else []
        representation_change_l2 = []
        representation_cosine = []
        dropped_blocks = []
        for block_index in range(self.num_blocks):
            before = queries
            dropped = False
            if block_index in set(getattr(self, "disabled_blocks", set())):
                dropped = True
            elif self.training and self.block_dropout > 0.0:
                dropped = bool(torch.rand((), device=queries.device).item() < self.block_dropout)
            if not dropped:
                layer = self.layers[0] if self.shared_block_weights else self.layers[block_index]
                if capture:
                    queries, weights = layer(queries, flat_tokens, flat_mask, need_weights=True)
                    attention_weights.append(weights.detach().float().cpu())
                else:
                    queries = layer(queries, flat_tokens, flat_mask)
            dropped_blocks.append(bool(dropped))
            if capture:
                early_logits.append(self.score_head(queries).squeeze(-1).detach().float().cpu())
                representations.append(queries.detach().float().cpu())
                delta = (queries - before).detach().float()
                representation_change_l2.append(float(delta.norm(dim=-1).mean().cpu().item()))
                before_flat = before.detach().float().reshape(-1, before.shape[-1])
                after_flat = queries.detach().float().reshape(-1, queries.shape[-1])
                representation_cosine.append(
                    float(torch.nn.functional.cosine_similarity(before_flat, after_flat, dim=-1).mean().cpu().item())
                )
        ...
        if capture:
            self.last_diagnostics = {
                ...
                "roles": int(roles),
                "tokens_per_role": int(tokens),
                "shared_block_weights": bool(self.shared_block_weights),
                "dropped_blocks": dropped_blocks,
                "early_logits": torch.stack(early_logits, dim=0) if early_logits else torch.empty(0),
                "attention_weights": torch.stack(attention_weights, dim=0) if attention_weights else torch.empty(0),
                "candidate_representations": torch.stack(representations, dim=0) if representations else torch.empty(0),
                "representation_change_l2": representation_change_l2,
                "representation_cosine": representation_cosine,
                "flat_token_mask": flat_mask.detach().cpu(),
            }
        return logits
...
def make_candidate_query_coordinator(
    ...
    if config.family in {"candidate_token_cross_attention", "stacked_candidate_token_cross_attention"}:
...
class _CrossAttentionBlock(nn.Module):
    def __init__(
        ...
        self.norm1 = nn.LayerNorm(model_dim) if use_layer_norm else nn.Identity()
        self.norm2 = nn.LayerNorm(model_dim) if use_layer_norm else nn.Identity()
    ...
    def forward(
        ...
        query = query + attended if self.use_residual_update else attended
        if self.use_feedforward:
            feed_forward = self.feed_forward(self.norm2(query))
            query = query + feed_forward if self.use_residual_update else feed_forward
        if need_weights:

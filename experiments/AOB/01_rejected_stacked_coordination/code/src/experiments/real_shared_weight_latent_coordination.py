...
class SharedClonedAgentSystem(nn.Module):
    ...
    def collect_clone_representations(
        ...
        if condition == "role_token_states_shuffled_across_examples" and raw_pooled.shape[0] > 1:
            rng = np.random.default_rng(seed + 59_711)
            shuffled_states = token_states.clone()
            shuffled_mask = token_mask.clone()
            for role_index in range(self.n_roles):
                perm = _non_identity_permutation(raw_pooled.shape[0], rng)
                index = torch.as_tensor(perm, dtype=torch.long, device=raw_pooled.device)
                shuffled_states[:, role_index, :, :] = token_states.index_select(0, index)[:, role_index, :, :]
                shuffled_mask[:, role_index, :] = token_mask.index_select(0, index)[:, role_index, :]
            token_states = shuffled_states
            token_mask = shuffled_mask
...
def fit_latent_system(
    ...
    candidate_query_families = {
        ...
        "stacked_candidate_token_cross_attention",
        "bilinear_candidate",

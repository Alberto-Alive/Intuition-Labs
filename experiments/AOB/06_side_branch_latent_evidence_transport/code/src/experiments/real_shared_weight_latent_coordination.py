...
class SharedClonedAgentSystem(nn.Module):
    ...
    def forward(
        ...
        if condition in {"physical_order_shuffled_roles_preserved", "physical_order_shuffled_roles_avenues_preserved"}:
        ...
        if condition == "physical_order_shuffled_roles_avenues_preserved":
            token_order = _token_permutation(
                readouts["token_states"].shape[0],
                readouts["token_states"].shape[1],
                readouts["token_states"].shape[2],
                rng,
                readouts["token_states"].device,
            )
            readouts["token_states"] = _gather_token_axis(readouts["token_states"], token_order)
            readouts["token_mask"] = _gather_token_axis(readouts["token_mask"], token_order)
        ...
        audit = {
            ...
            "physical_role_avenue_order_invariance_condition": condition == "physical_order_shuffled_roles_avenues_preserved",
        }
...
def fit_latent_system(
    ...
    transport_aux_loss_values: List[float] = []
    best_state = None
    best_dev = -1.0
    stale = 0
    ...
    for epoch in range(training_config.epochs):
        ...
        for batch_ids in _batches(order, training_config.batch_size):
            ...
            with _autocast_context(device, training_config.mixed_precision):
                ...
                transport_aux_loss = torch.zeros((), dtype=main_loss.dtype, device=main_loss.device)
                auxiliary_loss = getattr(coordinator, "auxiliary_loss", None)
                if callable(auxiliary_loss):
                    transport_aux_loss = auxiliary_loss(labels).to(dtype=main_loss.dtype, device=main_loss.device)
                transport_aux_loss_values.append(float(transport_aux_loss.detach().float().cpu().item()))
                total_loss = main_loss + aux_weight * aux_loss + regularization_loss + transport_aux_loss
            ...
            if first_backward is None:
                ...
                "loss_backward_reaches_transport": bool(
                    message_config.coordinator_family == "latent_evidence_transport" and coordinator_grad_norm > 0.0
                ),
        ...
        history.append(
            {
                ...
                "aux_weight": _auxiliary_loss_weight(epoch, message_config),
                "transport_aux_loss": _mean(transport_aux_loss_values[-max(1, len(order) // max(1, training_config.batch_size)) :]),
            }
        )
    ...
    audit = {
        ...
        "uses_latent_evidence_transport": message_config.coordinator_family == "latent_evidence_transport",
        "latent_evidence_transport_config": (
            coordinator.transport_audit() if hasattr(coordinator, "transport_audit") and callable(getattr(coordinator, "transport_audit")) else {}
        ),
        "uses_private_cue_aux": message_config.use_private_cue_aux,
        "aux_loss_weight": message_config.aux_loss_weight,
        "transport_aux_loss_mean": _mean(transport_aux_loss_values),
...
def _token_permutation(batch_size: int, n_roles: int, n_tokens: int, rng: np.random.Generator, device: torch.device) -> torch.Tensor:
    rows = np.zeros((batch_size, n_roles, n_tokens), dtype=np.int64)
    base = np.arange(n_tokens)
    for batch_id in range(batch_size):
        for role_id in range(n_roles):
            perm = rng.permutation(n_tokens)
            if n_tokens > 1 and np.array_equal(perm, base):
                perm = np.roll(perm, 1)
            rows[batch_id, role_id] = perm
    return torch.as_tensor(rows, dtype=torch.long, device=device)


def _gather_token_axis(tensor: torch.Tensor, order: torch.Tensor) -> torch.Tensor:
    if tensor.shape[:3] != order.shape:
        raise ValueError(f"token gather shape mismatch: tensor={tuple(tensor.shape)} order={tuple(order.shape)}")
    index = order

...
_INTEGRATED_COORDINATOR_CONTROL_MODES = {
...
class SharedClonedAgentSystem(nn.Module):
    ...
    def collect_clone_representations(
        ...
        for role_index in range(self.n_roles):
            if self.visible_explicit_evidence:
                ...
                texts = [format_clone_prompt(example, role_index) for example in examples]
            ...
            role_pooled.append(readouts["pooled"])
            role_msg.append(readouts["msg"])
            role_evidence.append(readouts.get("evidence", readouts["msg"]))
            role_tokens.append(readouts["token_states"])
            role_token_masks.append(readouts["token_mask"])
        raw_pooled = torch.stack(role_pooled, dim=1)
        msg = torch.stack(role_msg, dim=1)
        evidence = torch.stack(role_evidence, dim=1)
        token_states = torch.stack(role_tokens, dim=1)
        token_mask = torch.stack(role_token_masks, dim=1)
        if role_ids is None:
    ...
    def forward(
        ...
        integrated_control = condition if condition in _INTEGRATED_COORDINATOR_CONTROL_MODES else "none"
        previous_control = getattr(self.coordinator, "control_mode", "none")
        if hasattr(self.coordinator, "control_mode"):
            setattr(self.coordinator, "control_mode", integrated_control)
        if integrated_control in {"role_token_ablation", "cross_stream_attention_disabled"}:
            clone_activations = torch.zeros_like(clone_activations)
            readouts["message"] = clone_activations
            readouts["raw_pooled"] = torch.zeros_like(readouts["raw_pooled"])
            readouts["msg"] = torch.zeros_like(readouts["msg"])
            readouts["evidence"] = torch.zeros_like(readouts["evidence"])
            readouts["active_message"] = torch.zeros_like(readouts["active_message"])
            readouts["token_states"] = torch.zeros_like(readouts["token_states"])
        ...
        if bool(getattr(self.coordinator, "requires_candidate_features", False)):
            ...
            if integrated_control == "candidate_token_ablation":
                candidate_features = torch.zeros_like(candidate_features)
        ...
        if hasattr(self.coordinator, "control_mode"):
            setattr(self.coordinator, "control_mode", previous_control)
        audit = {
            ...
            "integrated_control_mode": integrated_control,
            "coordinator_attention_mask_audit": getattr(self.coordinator, "last_mask_audit", None),
            "coordinator_attention_stats": getattr(self.coordinator, "last_attention_stats", None),
            "coordinator_representation_change": getattr(self.coordinator, "last_representation_change", None),
        }
...
def fit_latent_system(
    ...
    visible_explicit_evidence: bool = False,
    message_config: MessageChannelConfig | None = None,
    repair_uniform_conditions: Sequence[str] | None = None,
    ...
    candidate_query_families = {
        ...
        "integrated_bridge_late",
        "integrated_summary_late",
        "integrated_router_late",
        "integrated_router_plus_tokens",
        "integrated_candidate_guided",
        "integrated_candidate_guided_late",
        "integrated_early_mixing_isolated",
        "integrated_early_mixing",
        "integrated_late_mixing_only",
    }
    ...
    repair_uniform_conditions = tuple(str(value) for value in (repair_uniform_conditions or ()))
    repair_uniform_weight = max(0.0, float(repair_uniform_weight))
    repair_uniform_controls_per_batch = max(0, int(repair_uniform_controls_per_batch))
    repair_candidate_order_shuffle_weight = max(0.0, float(repair_candidate_order_shuffle_weight))
    ...
    for epoch in range(training_config.epochs):
        ...
        for batch_ids in _batches(order, training_config.batch_size):
            ...
            with _autocast_context(device, training_config.mixed_precision):
                ...
                repair_loss = _repair_training_loss(
                    system=system,
                    batch_examples=batch_examples,
                    epoch=epoch,
                    step=step,
                    seed=seed,
                    uniform_conditions=repair_uniform_conditions,
                    uniform_weight=repair_uniform_weight,
                    uniform_controls_per_batch=repair_uniform_controls_per_batch,
                    candidate_order_shuffle_weight=repair_candidate_order_shuffle_weight if condition == "none" else 0.0,
                    mixed_precision=training_config.mixed_precision,
                    device=device,
                )
                total_loss = main_loss + aux_weight * aux_loss + regularization_loss + repair_loss
    ...
    audit = {
        ...
        "mixed_precision": training_config.mixed_precision,
        "repair_uniform_conditions": list(repair_uniform_conditions),
        "repair_uniform_weight": repair_uniform_weight,
        "repair_uniform_controls_per_batch": repair_uniform_controls_per_batch,
        "repair_candidate_order_shuffle_weight": repair_candidate_order_shuffle_weight,
...
def _repair_training_loss(
    system: SharedClonedAgentSystem,
    batch_examples: Sequence[MultiViewTaskExample],
    epoch: int,
    step: int,
    seed: int,
    uniform_conditions: Sequence[str],
    uniform_weight: float,
    uniform_controls_per_batch: int,
    candidate_order_shuffle_weight: float,
    mixed_precision: str,
    device: str,
) -> torch.Tensor:
    reference = next(system.parameters())
    total = torch.zeros((), dtype=reference.dtype, device=reference.device)
    if candidate_order_shuffle_weight > 0.0:
        shuffled = apply_example_control(batch_examples, "candidate_order_shuffled", seed=seed + 610_000 + epoch * 10_000 + step)
        logits, _audit, _acts = system(shuffled, condition="none", seed=seed + 620_000 + epoch)
        labels = _labels_tensor(shuffled, None, logits.device)
        total = total + float(candidate_order_shuffle_weight) * F.cross_entropy(logits, labels)
    if uniform_weight <= 0.0 or not uniform_conditions:
        return total
    if uniform_controls_per_batch <= 0:
        selected = list(uniform_conditions)
    else:
        selected = [
            uniform_conditions[(epoch + step + offset) % len(uniform_conditions)]
            for offset in range(min(uniform_controls_per_batch, len(uniform_conditions)))
        ]
    for offset, repair_condition in enumerate(selected):
        examples, forward_condition = _repair_uniform_batch(batch_examples, repair_condition, seed + 630_000 + epoch * 10_000 + step + offset)
        with _autocast_context(device, mixed_precision):
            logits, _audit, _acts = system(examples, condition=forward_condition, seed=seed + 640_000 + epoch + offset)
            total = total + float(uniform_weight) * _uniform_prediction_loss(logits) / max(1, len(selected))
    return total


def _repair_uniform_batch(
    batch_examples: Sequence[MultiViewTaskExample],
    repair_condition: str,
    seed: int,
) -> tuple[Sequence[MultiViewTaskExample], str]:
    if repair_condition == "candidate_only":
        return _candidate_only_training_examples(batch_examples), "none"
    if repair_condition == "view_masked_candidates_visible":
        return apply_example_control(batch_examples, "view_masked", seed=seed), "none"
    if repair_condition in {"view_masked", "candidate_order_shuffled", "randomized_labels"}:
        return apply_example_control(batch_examples, repair_condition, seed=seed), "none"
    return batch_examples, repair_condition


def _candidate_only_training_examples(examples: Sequence[MultiViewTaskExample]) -> List[MultiViewTaskExample]:
    out = []
    for example in examples:
        views = tuple(
            replace(
                view,
                text=(
                    "Program-analysis artifact hidden for candidate-only repair training.\n"
                    "No schema field or evidence value is available."
                ),
                source_type=f"{view.source_type}:stage6_repair_candidate_only_view_hidden",
            )
...
def _uniform_prediction_loss(logits: torch.Tensor) -> torch.Tensor:

...
RESULTS_PATH = Path("results/stage5_multi_avenue_search_results.json")
AUDIT_PATH = Path("results/stage5_multi_avenue_search_audit.jsonl")
REPORT_PATH = Path("reports/STAGE5_MULTI_AVENUE_SEARCH.md")
CHECKPOINT_DIR = Path("results/stage5_multi_avenue_search_checkpoints")
BASELINE_4X1 = {
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Stage 5 multi-avenue role expansion search.")
    ...
    print(
        "stage5: wrote {results}, {audit}, {report}; final_passed={passed}".format(
            results=RESULTS_PATH,
            audit=AUDIT_PATH,
            report=REPORT_PATH,
            passed=bool(result.get("summary", {}).get("passed_stage5_final_gates", False)),
        )
    )
...
def run_stage5(
    ...
    config = _stage5_config(base_config)
    ...
    if recompute_preflight or "pre_run" not in result:
        ...
        if bool(config.get("stop_on_preflight_failure", True)) and not bool(result["pre_run"]["summary"]["passes"]):
            print("stage5: pre-run shortcut diagnostics failed; search not started")
            return result
    ...
    for phase in phase_order[: max_index + 1]:
        ...
        if phase == "cheap":
            ...
            if not phase_variants:
                print("stage5 medium: no cheap-stage variant passed selection gates")
                break
        else:
            phase_variants = _selected_variants(result, "medium", top_k=1, require_baseline_improvement=True)
            if not phase_variants:
                print("stage5 final: no medium-stage variant passed selection and baseline-improvement gates")
                break
...
def _run_phase(
    ...
    for seed in stage.seeds:
        if all((int(seed), variant.name) in completed for variant in variants):
            print(f"stage5 {phase} seed={seed}: all variant rows already completed; skipping")
            continue
        print(f"stage5 {phase} seed={seed}: building fixed schema-aware splits")
        ...
        print(f"stage5 {phase} seed={seed}: fitting required baselines")
        ...
        for variant in variants:
            if (int(seed), variant.name) in completed:
                print(f"stage5 {phase} seed={seed}: existing completed row for {variant.name}; skipping")
                continue
            print(f"stage5 {phase} seed={seed}: fitting {variant.name}")
            row_completed = False
            for batch_size in fallbacks:
                ...
                try:
                    ...
                    row_completed = True
                    print(
                        "stage5 {phase} seed={seed} {name}: trainable={trainable:.4f} frozen={frozen:.4f} delta={delta:.4f}".format(
                            phase=phase,
                            seed=seed,
                            name=variant.name,
...
def _run_variant_seed(
    ...
    checkpoints = _save_stage5_checkpoints(
        config=config,
        phase=phase,
        seed=seed,
        stage=stage,
    ...
    row = {
        "stage": "stage5_multi_avenue_search",
        "phase": phase,
        "seed": int(seed),
        "candidate": variant.name,
        ...
        "avenue_audit": _avenue_audit(fit["trainable"], splits["test"], seed, variant.num_avenues),
    }
    row["success_criteria"] = _row_criteria(row, config)
    return row


def _fit_variant_methods(candidate, variant: Stage5Variant, stage: StageConfig, splits, seed: int, device: str, config: Dict[str, object], include_randomized: bool) -> Dict[str, object]:
    ...
    trainable = fit_latent_system(
        ...
        method=f"stage5_{candidate.name}__trainable",
    ...
    frozen = fit_latent_system(
        ...
        method=f"stage5_{candidate.name}__frozen",
        message_config=candidate.message_config,
    )
    randomized = None
    if include_randomized:
        randomized = fit_latent_system(
            ...
            device=device,
            trainable_agent=True,
            method=f"stage5_{candidate.name}__randomized_labels",
...
def _variant_candidate(stage: StageConfig, variant: Stage5Variant):
    ...
    coord = replace(
        _coordinator_config(stage, family="candidate_token_cross_attention", num_layers=1, dropout=float(variant.dropout)),
...
def _variants() -> List[Stage5Variant]:
    return [
        Stage5Variant(
            name="baseline_4x1_candidate_token_direct_lr3e4_clip1",
            description="Stage 4 candidate-token-direct baseline rerun on search splits.",
            num_avenues=1,
            avenue_prompt_mode="single",
            include_in_search=True,
        ),
        Stage5Variant(
            name="avenue2_goal_direct_lr3e4_clip1",
            description="Four roles with two goal-directed evidence avenues per role.",
            num_avenues=2,
            avenue_prompt_mode="goal",
        ),
        Stage5Variant(
            name="avenue4_goal_direct_lr3e4_clip1",
            description="Four roles with four goal-directed evidence avenues per role.",
            num_avenues=4,
            avenue_prompt_mode="goal",
        ),
        Stage5Variant(
            name="avenue4_goal_dropout05_lr3e4_clip1",
            description="Four goal-directed avenues with whole-avenue dropout 0.05.",
            num_avenues=4,
            avenue_prompt_mode="goal",
            avenue_dropout=0.05,
        ),
        Stage5Variant(
            name="avenue4_goal_topk4_lr3e4_clip1",
            description="Four goal-directed avenues with top-k avenue routing over four role-avenue groups.",
            num_avenues=4,
            avenue_prompt_mode="goal",
            avenue_topk=4,
        ),
        Stage5Variant(
            name="avenue2_schema_value_direct_lr3e4_clip1",
            description="Two avenues that vary schema/value analysis framing while preserving schema-aware evidence.",
            num_avenues=2,
            avenue_prompt_mode="schema_value_split",
        ),
    ]


def _selected_variants(result: Dict[str, object], phase: str, top_k: int, require_baseline_improvement: bool = False) -> List[Stage5Variant]:
    summaries = _phase_variant_summaries(result.get("phases", {}).get(phase, []))
    valid = [row for row in summaries if row.get("selection_valid") and not str(row["candidate"]).startswith("baseline_4x1")]
...
def _stage5_config(base: Dict[str, object]) -> Dict[str, object]:
    ...
    out["architecture"] = "stage5_multi_avenue_candidate_token_direct"
    out["architecture_changes"] = "bounded_multi_avenue_search_only"
    out["dataset"] = BALANCED_34B_DATASET_SOURCE
    out["dataset_config"] = {
        **dict(out.get("dataset_config", {})),
        "dataset_source": BALANCED_34B_DATASET_SOURCE,
        "generator_version": "real_import_restore_candidate_balanced_34b_stage5_multi_avenue",
        "candidate_representation": BALANCED_34B_CANDIDATE_REPRESENTATION,
    }
    out["stage5"] = {
        "cheap": {
            "name": "stage5a_cheap_search",
            "n_train": 512,
            "n_dev": 256,
            "n_test": 256,
            "seeds": [0, 1, 2],
        ...
        "medium": {
            "name": "stage5b_medium_validation",
            "n_train": 1024,
            "n_dev": 512,
            "n_test": 512,
            "seeds": [0, 1, 2, 3, 4],
    ...
    out["stage"] = out["stage5"]["cheap"]
    return out


def _stage_for_phase(config: Dict[str, object], phase: str) -> StageConfig:
    return _stage_from_config({**config, "stage": dict(config["stage5"][phase])})
    ...
    if not recompute:
        ...
        if stage4_path.exists():
            ...
            if pre_run:
                ...
                return pre_run
    pre_config = {**config, "stage": dict(config["stage5"]["final"])}
...
def _metadata(config: Dict[str, object], base_config_path: Path, hardware: Dict[str, object], device: str) -> Dict[str, object]:
    return {
        ...
        "selection_rule": "Variant selection uses dev metrics from cheap and medium phases only; final test seeds are not used for tuning.",
...
def _summary(result: Dict[str, object]) -> Dict[str, object]:
    ...
    return {
        ...
        "passed_stage5_final_gates": bool(criteria and all(bool(item["pass"]) for item in criteria)),
    }
...
def _final_criteria(rows: Sequence[Dict[str, object]], config: Dict[str, object]) -> List[Dict[str, object]]:
    ...
    avenue_ok = all(bool(row.get("avenue_audit", {}).get("no_single_avenue_explains_result", True)) for row in rows)
    return [
        ...
        {"criterion": "no single avenue explains the result", "pass": avenue_ok, "value": [row.get("avenue_audit", {}) for row in rows]},
        {"criterion": "multi-avenue improves over Stage 4 baseline mean or robustness", "pass": _final_improves_stage4(rows), "value": {"stage4_baseline": BASELINE_4X1, "stage5_mean": train_mean, "stage5_min": min(trainable), "stage5_delta": _mean(deltas), "stage5_std": pstdev(trainable) if len(trainable) > 1 else 0.0}},
...
def _render_report(result: Dict[str, object]) -> str:
    ...
    for variant in result.get("search_space", []):
        lines.append(
            "| {name} | {avenues} | {prompt} | {topk} | {dropout:.2f} | {lr:.4g} | {clip:.2f} |".format(
                name=variant["name"],
                avenues=int(variant["num_avenues"]),
                prompt=variant["avenue_prompt_mode"],
                topk=int(variant["avenue_topk"]),
                dropout=float(variant["avenue_dropout"]),
                lr=float(variant["lr"]),
                clip=float(variant["gradient_clip_norm"]),
            )
        )
    lines.extend(["", "## Pre-Run Controls", ""])
    pre = result.get("pre_run", {})
    lines.append(f"- Source: `{pre.get('source', 'computed_for_stage5')}`")
    ...
    for phase, rows in summary.get("phase_summaries", {}).items():
        lines.extend([f"### {phase.title()}", "", "| variant | n | dev train | dev frozen | dev delta | min dev | weak <0.60 | controls | invariance | selected-valid |", "|---|---:|---:|---:|---:|---:|---:|---|---|---|"])
        for row in rows:
            lines.append(
                "| {candidate} | {n} | {train:.4f} | {frozen:.4f} | {delta:.4f} | {minv:.4f} | {weak} | `{controls}` | `{inv}` | `{valid}` |".format(
                    ...
                    minv=float(row["min_dev_trainable"]),
                    weak=int(row["weak_dev_seeds_below_060"]),
    ...
    final_rows = [row for row in result.get("phases", {}).get("final", []) if row.get("status") == "completed"]
    if final_rows:
        lines.extend(["| seed | variant | trainable | frozen | delta | physical-order | candidate-order | max single avenue |", "|---:|---|---:|---:|---:|---:|---:|---:|"])
        for row in final_rows:
            test = row["test_accuracy"]
            avenue = row.get("avenue_audit", {})
            lines.append(
                f"| {row['seed']} | {row['candidate']} | {float(test['trainable']):.4f} | {float(test['frozen']):.4f} | {float(row['test_delta']):.4f} | {float(test.get('physical_order_shuffled_roles_preserved', 0.0)):.4f} | {float(test.get('candidate_order_shuffled_with_gold_remap', 0.0)):.4f} | {float(avenue.get('max_single_avenue_accuracy', 0.0)):.4f} |"
            )
    else:
        lines.append("- Final validation has not run; this report does not claim Stage 5 success.")
    ...
    if failed:
        lines.append("- Final Stage 5 success is not claimed.")
        lines.append("- Failed final gates: `" + ", ".join(str(item["criterion"]) for item in failed) + "`.")
        if any(str(item.get("criterion")) == "no single avenue explains the result" for item in failed):
            lines.append("- The selected multi-avenue variant often keeps near-full accuracy when only one avenue is visible, so the final result is not clean evidence that multiple avenues jointly drive the gain.")
    else:
    ...
    lines.extend(
        [
            ...
            "",
            f"- Stage 5 final gates passed: `{bool(summary.get('passed_stage5_final_gates', False))}`.",

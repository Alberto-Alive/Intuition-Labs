...
class MessageChannelConfig:
    ...
    avenue_bottleneck_dim: int = 0
    single_avenue_adversarial_weight: float = 0.0
    single_avenue_adversarial_num_classes: int = 8
    redundancy_loss_type: str = "none"
    redundancy_loss_weight: float = 0.0
    anti_dominance_weight: float = 0.0
    anti_dominance_mode: str = "off"
...
def _grad_reverse(value: torch.Tensor, scale: float = 1.0) -> torch.Tensor:
    return _GradientReverse.apply(value, float(scale))


class AvenueBottleneck(nn.Module):
    def __init__(self, input_dim: int, bottleneck_dim: int) -> None:
        super().__init__()
        self.net = nn.Sequential(
            nn.LayerNorm(input_dim),
            nn.Linear(input_dim, int(bottleneck_dim)),
            nn.GELU(),
            nn.Linear(int(bottleneck_dim), input_dim),
        )

    def forward(self, token_states: torch.Tensor) -> torch.Tensor:
        return self.net(token_states)
...
class SharedClonedAgentSystem(nn.Module):
    def __init__(
        ...
        self.avenue_bottlenecks: nn.ModuleList | None = None
        self.single_avenue_adversaries: nn.ModuleList | None = None
        ...
        bottleneck_dim = int(self.message_config.avenue_bottleneck_dim)
        if avenue_count > 1 and bottleneck_dim > 0:
            self.avenue_bottlenecks = nn.ModuleList(
                [AvenueBottleneck(shared_agent.output_dim, bottleneck_dim) for _ in range(avenue_count)]
            )
        if avenue_count > 1 and float(self.message_config.single_avenue_adversarial_weight) > 0.0:
            self.single_avenue_adversaries = nn.ModuleList(
                [
                    nn.Sequential(
                        nn.LayerNorm(shared_agent.output_dim),
                        nn.Linear(shared_agent.output_dim, int(self.message_config.single_avenue_adversarial_num_classes)),
                    )
                    for _ in range(avenue_count)
                ]
            )
        self.last_avenue_summaries: torch.Tensor | None = None
        self.last_adversarial_logits: torch.Tensor | None = None
        self.last_dominance_weights: torch.Tensor | None = None
    ...
    def collect_clone_representations(
        ...
        if self.avenue_bottlenecks is not None:
            token_states_full = torch.stack(
                [
                    bottleneck(token_states_full[:, :, avenue_index, :, :])
                    for avenue_index, bottleneck in enumerate(self.avenue_bottlenecks)
                ],
                dim=2,
            )
    ...
    def forward(
        ...
        self.last_adversarial_logits = None
        self.last_avenue_summaries = None
        ...
        if readouts["token_mask"].dim() == 4 and str(condition).startswith("avenue_subset_"):
            suffix = str(condition).split("avenue_subset_", 1)[1]
            selected = {
                int(value)
                for value in suffix.replace(",", "-").split("-")
                if value.strip().isdigit()
            }
            avenue_mask = torch.zeros_like(readouts["token_mask"], dtype=torch.bool)
            for avenue_index in selected:
                if 0 <= avenue_index < readouts["token_mask"].shape[2]:
                    avenue_mask[:, :, avenue_index, :] = readouts["token_mask"][:, :, avenue_index, :].bool()
            if avenue_mask.any(dim=(2, 3)).all():
        ...
        self.last_avenue_summaries = self._avenue_summaries(readouts)
        if self.single_avenue_adversaries is not None and self.last_avenue_summaries is not None:
            self.last_adversarial_logits = torch.stack(
                [
                    head(_grad_reverse(self.last_avenue_summaries[:, avenue_index, :], 1.0))
                    for avenue_index, head in enumerate(self.single_avenue_adversaries)
                ],
                dim=1,
            )
        ...
        self.last_dominance_weights = getattr(self.coordinator, "last_avenue_attention", None)
        audit = {
            ...
            "avenue_bottleneck_dim": self.message_config.avenue_bottleneck_dim,
            "uses_single_avenue_adversaries": self.single_avenue_adversaries is not None,
            "single_avenue_adversarial_weight": self.message_config.single_avenue_adversarial_weight,
            "redundancy_loss_type": self.message_config.redundancy_loss_type,
            "redundancy_loss_weight": self.message_config.redundancy_loss_weight,
            "anti_dominance_mode": self.message_config.anti_dominance_mode,
            "anti_dominance_weight": self.message_config.anti_dominance_weight,
    ...
    def _avenue_summaries(self, readouts: Dict[str, torch.Tensor]) -> torch.Tensor | None:
        token_states = readouts.get("token_states")
        token_mask = readouts.get("token_mask")
        if token_states is None or token_mask is None:
            return None
        if token_states.dim() == 4:
            valid = token_mask.bool().to(dtype=token_states.dtype)
            summary = (token_states * valid.unsqueeze(-1)).sum(dim=(1, 2))
            denom = valid.sum(dim=(1, 2)).clamp(min=1.0).unsqueeze(-1)
            return (summary / denom).unsqueeze(1)
        if token_states.dim() != 5:
            return None
        valid = token_mask.bool().to(dtype=token_states.dtype)
        summary = (token_states * valid.unsqueeze(-1)).sum(dim=(1, 3))
        denom = valid.sum(dim=(1, 3)).clamp(min=1.0).unsqueeze(-1)
        return summary / denom
...
def fit_latent_system(
    ...
    candidate_query_families = {
        ...
        "candidate_token_gated_attention",
        "candidate_token_product_of_experts",
        "bilinear_candidate",
    ...
    initial_avenue_bottlenecks = _flat_module_parameters(system.avenue_bottlenecks) if system.avenue_bottlenecks is not None else torch.zeros(0)
    initial_single_avenue_adversaries = _flat_module_parameters(system.single_avenue_adversaries) if system.single_avenue_adversaries is not None else torch.zeros(0)
    ...
    avenue_bottleneck_grad_norms: List[float] = []
    single_avenue_adversary_grad_norms: List[float] = []
    best_state = None
    best_dev = -1.0
    stale = 0
    ...
    for epoch in range(training_config.epochs):
        ...
        for batch_ids in _batches(order, training_config.batch_size):
            ...
            with _autocast_context(device, training_config.mixed_precision):
                ...
                adv_loss = _single_avenue_adversarial_loss(system.last_adversarial_logits, labels)
                complementarity_loss = _complementarity_regularization_loss(system.last_avenue_summaries, message_config)
                dominance_loss = _anti_dominance_loss(system.last_dominance_weights, message_config)
                adv_loss = adv_loss.to(device=main_loss.device, dtype=main_loss.dtype)
                complementarity_loss = complementarity_loss.to(device=main_loss.device, dtype=main_loss.dtype)
                dominance_loss = dominance_loss.to(device=main_loss.device, dtype=main_loss.dtype)
                total_loss = (
                    main_loss
                    + aux_weight * aux_loss
                    + regularization_loss
                    + float(message_config.single_avenue_adversarial_weight) * adv_loss
            ...
            avenue_bottleneck_grad_norm = _grad_norm(system.avenue_bottlenecks.parameters()) if system.avenue_bottlenecks is not None else 0.0
            single_avenue_adversary_grad_norm = _grad_norm(system.single_avenue_adversaries.parameters()) if system.single_avenue_adversaries is not None else 0.0
            ...
            avenue_bottleneck_grad_norms.append(avenue_bottleneck_grad_norm)
            single_avenue_adversary_grad_norms.append(single_avenue_adversary_grad_norm)
            if first_backward is None:
                ...
                first_backward = {
                    **forward_audit,
                    ...
                    "loss_backward_reaches_avenue_bottlenecks": bool(avenue_bottleneck_grad_norm > 0.0),
                    "loss_backward_reaches_single_avenue_adversaries": bool(single_avenue_adversary_grad_norm > 0.0),
        ...
        history.append(
            {
                ...
                "aux_weight": _auxiliary_loss_weight(epoch, message_config),
                "single_avenue_adversarial_weight": float(message_config.single_avenue_adversarial_weight),
                "redundancy_loss_weight": float(message_config.redundancy_loss_weight),
                "anti_dominance_weight": float(message_config.anti_dominance_weight),
            }
        )
    ...
    avenue_bottleneck_delta = (
        _parameter_delta(list(system.avenue_bottlenecks.named_parameters()), initial_avenue_bottlenecks)
        if system.avenue_bottlenecks is not None
        else 0.0
    )
    single_avenue_adversary_delta = (
        _parameter_delta(list(system.single_avenue_adversaries.named_parameters()), initial_single_avenue_adversaries)
        if system.single_avenue_adversaries is not None
        else 0.0
    )
    ...
    audit = {
        ...
        "avenue_bottleneck_grad_norm_mean": _mean(avenue_bottleneck_grad_norms),
        "avenue_bottleneck_grad_norm_std": _std(avenue_bottleneck_grad_norms),
        "single_avenue_adversary_grad_norm_mean": _mean(single_avenue_adversary_grad_norms),
        "single_avenue_adversary_grad_norm_std": _std(single_avenue_adversary_grad_norms),
        ...
        "avenue_bottleneck_parameter_delta": avenue_bottleneck_delta,
        "single_avenue_adversary_parameter_delta": single_avenue_adversary_delta,
        ...
        "loss_backward_reaches_avenue_bottlenecks": bool(first_backward.get("loss_backward_reaches_avenue_bottlenecks", False)),
        "loss_backward_reaches_single_avenue_adversaries": bool(first_backward.get("loss_backward_reaches_single_avenue_adversaries", False)),
...
def _single_avenue_adversarial_loss(logits: torch.Tensor | None, labels: torch.Tensor) -> torch.Tensor:
    if logits is None or logits.numel() == 0:
        return torch.zeros((), dtype=torch.float32, device=labels.device)
    targets = labels.unsqueeze(1).expand(-1, logits.shape[1]).reshape(-1)
    return F.cross_entropy(logits.reshape(-1, logits.shape[-1]), targets)


def _complementarity_regularization_loss(
    avenue_summaries: torch.Tensor | None,
    message_config: MessageChannelConfig,
) -> torch.Tensor:
    weight = float(message_config.redundancy_loss_weight)
    kind = str(message_config.redundancy_loss_type or "none").lower()
    if avenue_summaries is None or avenue_summaries.numel() == 0 or avenue_summaries.shape[1] <= 1 or weight <= 0.0 or kind == "none":
        device = avenue_summaries.device if avenue_summaries is not None else torch.device("cpu")
        return torch.zeros((), dtype=torch.float32, device=device)
    summaries = avenue_summaries.float()
    avenues = summaries.shape[1]
    off_diag = ~torch.eye(avenues, dtype=torch.bool, device=summaries.device)
    if kind == "cosine":
        normalized = F.normalize(summaries, dim=-1)
        sim = torch.einsum("bad,bcd->bac", normalized, normalized)
        loss = sim[:, off_diag].pow(2).mean()
    elif kind == "orthogonality":
        centered = summaries - summaries.mean(dim=1, keepdim=True)
        normalized = F.normalize(centered, dim=-1)
        gram = torch.einsum("bad,bcd->bac", normalized, normalized)
        loss = gram[:, off_diag].pow(2).mean()
    elif kind in {"covariance", "barlow", "barlow_style"}:
        flat = summaries.permute(1, 0, 2).reshape(avenues, -1)
        flat = flat - flat.mean(dim=1, keepdim=True)
        flat = F.normalize(flat, dim=1)
        corr = flat @ flat.t()
        loss = corr[off_diag].pow(2).mean()
    else:
        raise ValueError(f"unknown redundancy loss type: {message_config.redundancy_loss_type}")
    return torch.as_tensor(weight, dtype=avenue_summaries.dtype, device=avenue_summaries.device) * loss.to(dtype=avenue_summaries.dtype)


def _anti_dominance_loss(
    dominance_weights: torch.Tensor | None,
    message_config: MessageChannelConfig,
) -> torch.Tensor:
    weight = float(message_config.anti_dominance_weight)
    mode = str(message_config.anti_dominance_mode or "off").lower()
    if dominance_weights is None or dominance_weights.numel() == 0 or dominance_weights.shape[-1] <= 1 or weight <= 0.0 or mode == "off":
        device = dominance_weights.device if dominance_weights is not None else torch.device("cpu")
        return torch.zeros((), dtype=torch.float32, device=device)
    weights = dominance_weights.float()
    avenues = weights.shape[-1]
    concentration = weights.pow(2).sum(dim=-1) - (1.0 / float(avenues))
    multiplier = 2.0 if mode == "medium" else 1.0
    return torch.as_tensor(weight * multiplier, dtype=dominance_weights.dtype, device=dominance_weights.device) * concentration.clamp(min=0.0).mean().to(dtype=dominance_weights.dtype)

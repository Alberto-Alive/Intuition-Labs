...
from src.experiments.run_stage6b1_candidate_generation import (
    ...
    load_default_hf_public_sources,
    write_stage6b1_jsonl,
)


BENCHMARK = "stage6b1a_generator_reliability_ramp"
DEFAULT_OUTPUT_PATH = Path("results/stage6b1a_generated_candidates.jsonl")
DEFAULT_ATTEMPTS_PATH = Path("results/stage6b1a_generation_attempts.jsonl")
DEFAULT_AUDIT_PATH = Path("results/stage6b1a_generation_audit.json")
DEFAULT_REPORT_PATH = Path("reports/STAGE6B1A_GENERATOR_RELIABILITY_RAMP.md")
DEFAULT_TRAJECTORY_ROOT = Path("results/stage6b1a_trajectories")
DEFAULT_CANDIDATE_PATCH_ROOT = Path("results/stage6b1a_candidate_patches")
DEFAULT_PROMPT_ROOT = Path("results/stage6b1a_prompts")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Stage 6B.1a generator reliability ramp for SWE-bench candidate generation.")
    ...
    result = run_stage6b1a_generator_reliability_ramp(
    ...
    audit = result["audit"]
    print(
        "stage6b1a: wrote {output}, {attempts}, {audit_path}, {report}; rows={rows}; tasks_k8={tasks_k8}; gates_pass={gates}".format(
            output=args.output,
            ...
            gates=audit.get("quality_gates", {}).get("generator_reliability_quality_gates_pass", False),
        )
    )


def run_stage6b1a_generator_reliability_ramp(
    dataset_name: str,
    split: str,
    ...
    accepted, attempt_rows, build_audit = select_stage6b1a_candidates(
    ...
    report = render_stage6b1a_report(audit=audit, output_path=output_path, attempts_path=attempts_path, audit_path=audit_path, report_path=report_path)
...
def load_hf_public_attempt_sources(
    ...
    stage6b1_source_audit: Dict[str, object] = {"sources": [], "errors": []}
    stage6b1_attempts = load_default_hf_public_sources(task_ids, trajectory_root, stage6b1_source_audit)
    source_audit.setdefault("sources", []).extend(stage6b1_source_audit.get("sources", []))
    source_audit.setdefault("errors", []).extend(stage6b1_source_audit.get("errors", []))
    rows = []
    for index, attempt in enumerate(stage6b1_attempts):
        patches = _patch_candidates_from_value(attempt.model_patch, explicit_patch_field=False)
        if not patches:
            patches = [""]
        for patch_index, patch in enumerate(patches):
            rows.append(
                RawGenerationAttempt(
                    ...
                    attempt_id=f"hf-public-{index:08d}-{patch_index:02d}",
...
def _load_prediction_attempt_file(
    ...
    summary = {"source_name": source_name, "loaded": False, "records": 0, "candidate_attempts": 0, "terminal_no_patch_rows": 0, "error": None}
    rows: List[RawGenerationAttempt] = []
    try:
...
def _patch_candidates_from_value(value: object, explicit_patch_field: bool) -> List[str]:
    ...
    if isinstance(value, dict):
        patches: List[str] = []
        for key in ("model_patch", "output_patch", "generated_patch", "prediction", "diff", "patch", "content", "text"):
            if key in value:
                patches.extend(_patch_candidates_from_value(value.get(key), explicit_patch_field=explicit_patch_field))
        return patches
...
def _extract_patch_candidates_from_text(text: str) -> List[str]:
    ...
    if "diff --git " in text:
        patches.append(_clean_patch(text[text.find("diff --git ") :]))
    else:
...
def select_stage6b1a_candidates(
    ...
    for instance_id, task in task_records.items():
        ...
        if not task_attempts:
            attempt_rows.append(_task_skipped_row(task, max_attempts_per_task))
            continue
        attempts_used = 0
    ...
    for instance_id in task_records:
        ...
        for out_index, (attempt, patch, patch_hash, audit_row) in enumerate(selected):
            candidate_id = f"{instance_id}-stage6b1a-candidate-{out_index:02d}-{patch_hash[:12]}"
...
def run_patch_apply_check(
    ...
    with tempfile.TemporaryDirectory(prefix="stage6b1a_apply_") as temp_dir:
...
def build_stage6b1a_audit(
    ...
    quality_gates["generator_reliability_quality_gates_pass"] = all(
        bool(value)
        for key, value in quality_gates.items()
        if key not in {"selector_training_executed", "official_harness_evaluation_executed"}
    ) and quality_gates["selector_training_executed"] is False and quality_gates["official_harness_evaluation_executed"] is False
    return {
        "benchmark": BENCHMARK,
        ...
        "failure_taxonomy": {"terminal_statuses": sorted(TERMINAL_STATUSES), "status_counts": build_audit.get("terminal_status_counts", {})},
...
def render_stage6b1a_report(audit: Dict[str, object], output_path: Path, attempts_path: Path, audit_path: Path, report_path: Path) -> str:
    ...
    if not bool(gates.get("generator_reliability_quality_gates_pass", False)):
        lines.extend(
            [
                "",
                "## Current Limitation",
    ...
    lines.extend(
        [
            ...
            "## Next Step",
            "",
            f"When the gates pass, run Stage 6B.0 with `--source-jsonl {output_path}` and then the official SWE-bench Docker harness.",
            "",
            "No final model claim is made.",
            "",
        ]
...
def _build_selection_audit(
    ...
    rejection_counts = Counter(
        ...
        for row in attempt_rows
        if row.get("terminal_status") not in {"accepted", "task_skipped"}
    )
    ...
    evaluated_attempts = [row for row in attempt_rows if row.get("terminal_status") != "task_skipped"]
...
def _task_skipped_row(task: BenchmarkTask, max_attempts_per_task: int) -> Dict[str, object]:
    return {
        "instance_id": task.instance_id,
        "attempt_id": f"task-skipped-{_safe_name(task.instance_id)}",
        "generation_attempt_index": None,
        "terminal_status": "task_skipped",
        "candidate_id": None,
        ...
        "failure_message": f"no generation attempts available within max_attempts_per_task={max_attempts_per_task}",
    }
...
def _dedupe_patch_texts(values: Iterable[str]) -> List[str]:
    ...
    seen = set()
    for value in values:
        patch = _ensure_trailing_newline(_clean_patch(str(value or ""))) if str(value or "").strip() else ""

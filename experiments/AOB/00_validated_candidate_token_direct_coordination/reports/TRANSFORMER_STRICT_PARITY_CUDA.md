...
{
  "config": {
    "agent_view_log_path": "results/transformer_strict_parity_cuda_agent_views.jsonl",
    "controls": [
      "none"
    ],
    "coordinator_visible_log_path": "results/transformer_strict_parity_cuda_visible.jsonl",
    "cuda_memory_budget_gb": 16.0,
    "deterministic": true,
    "device": "cuda",
    "hidden_state_npz_path": "results/transformer_strict_parity_cuda_hidden_states.npz",
    "hidden_state_probe_name": "hidden_state_only_probe",
    "output_path": "results/transformer_strict_parity_cuda.json",
    "report_path": "reports/TRANSFORMER_STRICT_PARITY_CUDA.md",
  ...
  "config_path": "configs\\transformer_strict_parity_cuda.json",
  "device_used": "cuda",
  "hardware": {
    "cuda_available": true,
...
| transformer_strict | dev | none | activation_pca_mlp | 0.2875 | 0.0000 | 1 | 2324 |
...
| transformer_strict | test | none | activation_pca_mlp | 0.2750 | 0.0000 | 1 | 2324 |
...
| transformer_strict | activation_pca_mlp | 0.2750 | 0.0000 | 1 |
...
|---|---|---:|---|---:|
| transformer_strict | activation_pca_mlp accuracy | 0.2750 | [0.2750, 0.2750] | 1 |
...
| transformer_strict | correctness AUC activation_only | 0.4824 | [0.4715, 0.5003] | 4 |
| transformer_strict | correctness AUC text_output_only | 0.5000 | [0.5000, 0.5000] | 4 |
| transformer_strict | correctness AUC text_plus_activation | 0.4824 | [0.4715, 0.5003] | 4 |
...
| transformer_strict | none | activation_pca_mlp | 0.2750 | 0.0000 | 1 |
...
|---|---|---:|---:|---:|---:|
| transformer_strict | activation_pca_mlp | 0.2750 | 0.0000 | 1 | 2324 |
...
- `activation_pca_mlp` hidden+visible accuracy is 0.2750; interpret it alongside hidden-only and stability diagnostics because PCA/pooling variance remains material.
...
| activation_pca_mlp | 0.2750 | 0.0000 | [0.2750, 0.2750] | 1 |
...
|---|---|---:|---:|---:|
| transformer_strict | activation_only | 0.7833 | 0.4824 | 4 |
| transformer_strict | text_output_only | 0.7833 | 0.5000 | 4 |
| transformer_strict | text_plus_activation | 0.7833 | 0.4824 | 4 |
...
| transformer_strict | 0 | dev | none | activation_pca_mlp | 0.2875 | 2324 |
...
| transformer_strict | 0 | test | none | activation_pca_mlp | 0.2750 | 2324 |
...
- Transformer strict mechanism: captured local-transformer hidden states give hidden-only accuracy 0.2750 and hidden+output accuracy 0.2750, while output-only oracle accuracy is 0.2750. Mean individual-agent hidden label accuracy is 0.0000; this distinguishes combined private-evidence aggregation from single-agent final-label decoding.
...
- `transformer_strict`: hidden-only=0.2750, hidden+output=0.2750, output-only oracle=0.2750.
...
- `transformer_strict`: `activation_pca_mlp` scored 0.2750; strongest output-only reference scored 0.2833; delta -0.0083.
...
- `transformer_strict`: agent-correctness AUC text=0.5000, activation=0.4824, text+activation=0.4824.
